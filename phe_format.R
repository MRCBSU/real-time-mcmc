...
ICU <- list()
D <- list()
inf <- ar <- nni <- hosp <- icu <- deaths <- NULL
for(reg in regions){
...
write.csv(out, paste0(dir.string, "/internal_", str.scenario), row.names = FALSE)
