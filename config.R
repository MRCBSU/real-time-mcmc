...
google.data.date <- format(ymd("20220617"), format = "%Y%m%d")
...
if(use.previous.run.for.start){
    if(region.type == "NHS"){
        previous.run.to.use <- file.path(proj.dir, "model_runs", ifelse(hosp.flag, "20220606", "20220607"), paste0("Prev765SeroNHSBT_All_NHS", str.cutoff, "cutoff_IFR7bp_11wk2_prev14-0PHE_3dose_new_mprior_matrices_20220607", matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))                                      )
    } else if(region.type == "ONS")
        previous.run.to.use <- file.path(proj.dir, "model_runs", ifelse(hosp.flag, "20220606", "20220607"), paste0("Prev765SeroNHSBT_All_ONS", str.cutoff, "cutoff_IFR7bp_", ifelse(admissions_only.flag & !hosp.flag, "admissions_only_", ""), "11wk2_prev14-0PHE_3dose_new_mprior_matrices_20220607", matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                         )
}
...
date.prev <- lubridate::ymd("20220615")
...
str.date.vacc <- "20220616" ## Optional: if not specified will take the most recent data file.
vacc.lag <- 21
vac.overwrite <- FALSE
if(vacc.flag){
