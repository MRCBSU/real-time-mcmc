...
if(use.previous.run.for.start){
    if(region.type == "NHS"){
        if(str.cutoff == "60")
            previous.run.to.use <- file.path(proj.dir, "model_runs", "20220114", paste0("Prev620SeroNHSBT_All_NHS", str.cutoff, "cutoff_IFR6bp_11wk2_prev14-0PHE_matrices_20220114", matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                             )
        else previous.run.to.use <- file.path(proj.dir, "model_runs", "20220114", paste0("Prev620SeroNHSBT_All_NHS", str.cutoff, "cutoff_IFR6bp_11wk2_prev14-0PHE_matrices_20220114", matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                              )
    } else if(region.type == "ONS")
        previous.run.to.use <- file.path(proj.dir, "model_runs", "20220114", paste0("Prev620SeroNHSBT_All_ONS", str.cutoff, "cutoff_IFR6bp_11wk2_prev14-0PHE_matrices_20220114", matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                         )
}
