...
google.data.date <- format(ymd("20211231"), format = "%Y%m%d")
...
if(use.previous.run.for.start){
    if(region.type == "NHS"){
        if(str.cutoff == "60")
            previous.run.to.use <- file.path(proj.dir, "model_runs", "20211224", paste0(c("Prev599SeroNHSBT_All_NHS60cutoff_IFR6bp_18wk2_prev14-0PHE_matrices_20211223",
                                                                                          "Prev599SeroNHSBT_All_NHS60cutoff_IFR6bp_18wk2_prev14-0PHE_matrices_20211223"), matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                             )
        else previous.run.to.use <- file.path(proj.dir, "model_runs", "20211224", paste0(c("Prev599SeroNHSBT_All_NHS28cutoff_IFR6bp_18wk2_prev14-0PHE_matrices_20211223",
                                                                                           "Prev599SeroNHSBT_All_NHS28cutoff_IFR6bp_18wk2_prev14-0PHE_matrices_20211223"), matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                              )
    } else if(region.type == "ONS")
        previous.run.to.use <- file.path(proj.dir, "model_runs", "20211224", paste0(c("Prev599SeroNHSBT_All_ONS60cutoff_IFR6bp_18wk2_prev14-0PHE_matrices_20211223", # _stable_household_deaths_chain2",
                                                                                      "Prev599SeroNHSBT_All_ONS60cutoff_IFR6bp_18wk2_prev14-0PHE_matrices_20211223") # _stable_household_deaths")
...
date.prev <- lubridate::ymd("20211229")
...
str.date.vacc <- "20211230" ## Optional: if not specified will take the most recent data file.
vacc.lag <- 21
vac.overwrite <- FALSE
if(vacc.flag){
