...
if(use.previous.run.for.start){
    if(region.type == "NHS"){
        if(str.cutoff == "60")
            previous.run.to.use <- file.path(proj.dir, "model_runs", ifelse(hosp.flag, "20220318", "20220319"), paste0("Prev683SeroNHSBT_All_NHS", "28", "cutoff_IFR7bp_11wk2_prev14-0PHE_3dose_matrices_20220318", matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                             )
        else previous.run.to.use <- file.path(proj.dir, "model_runs", ifelse(hosp.flag, "20220318", "20220319"), paste0("Prev683SeroNHSBT_All_NHS", str.cutoff, "cutoff_IFR7bp_11wk2_prev14-0PHE_3dose_matrices_20220318", matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                              )
    } else if(region.type == "ONS")
        previous.run.to.use <- file.path(proj.dir, "model_runs", ifelse(hosp.flag, "20220318", "20220319"), paste0("Prev683SeroNHSBT_All_ONS", str.cutoff, "cutoff_IFR7bp_11wk2_prev14-0PHE_3dose_matrices_20220318", matrix.suffix, "_", ifelse(hosp.flag, "deaths", "admissions_no_deaths"), c("_chain2", ""))
                                         )
}
