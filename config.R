...
if(use.previous.run.for.start){
    if(region.type == "NHS"){
        previous.run.to.use <- file.path(proj.dir,
                                         "model_runs",
                                         ifelse(hosp.flag, "20221101", "20221104"),
                                         paste0("Prev912SeroNHSBT_All_NHS",
                                                str.cutoff,
                                                "cutoff_IFR9bp_",
        ...
        previous.run.to.use <- file.path(proj.dir,
                                         "model_runs",
                                         ifelse(hosp.flag, "20221101", "20221104"),
                                         paste0("Prev912SeroNHSBT_All_ONS",
                                                str.cutoff,
                                                "cutoff_IFR9bp_",
                                                ifelse(admissions_only.flag & !hosp.flag, "admissions_only_", ""),
                                                "11wk2_prev14-3PHE_4dose_new_mprior_",
...
date.prev <- lubridate::ymd("20221107")
...
str.date.vacc <- "20221108" ## Optional: if not specified will take the most recent data file.
vacc.lag <- 21
vac.overwrite <- FALSE
if(vacc.flag){
