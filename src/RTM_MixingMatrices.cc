...
void mixing_model_alloc(struct mixing_model &l_MIXMOD, const int nbreakpoints,
                        const int num_scalants, const int num_strata) {

    l_MIXMOD.num_breakpoints = nbreakpoints;
    l_MIXMOD.breakpoints =
        (nbreakpoints > 0) ? gsl_vector_int_alloc(nbreakpoints) : 0;
    ...
    for (int inti = 0; inti < nbreakpoints + 1; inti++) {
        ...
        l_MIXMOD.MIXMAT_param[inti] =
            gsl_matrix_int_alloc(num_strata, num_strata);
...
void mixing_model_free(struct mixing_model &l_MIXMOD) {
    ...
    if (l_MIXMOD.num_breakpoints > 0)
        gsl_vector_int_free(l_MIXMOD.breakpoints);
}
...
void mixing_model_memcpy(mixing_model &mix_dest, const mixing_model &mix_src) {
    ...
    for (register int i = 0; i <= mix_src.num_breakpoints; i++) {
        ...
        gsl_matrix_int_memcpy(mix_dest.MIXMAT_param[i],
                              mix_src.MIXMAT_param[i]);
        if (mix_src.evector_MIXMAT_normalised != 0)
            gsl_vector_memcpy(mix_dest.evector_MIXMAT_normalised[i],
                              mix_src.evector_MIXMAT_normalised[i]);
    }
}
...
int maximal_index(const mixing_model src_mix) {
    ...
    for (int int_i = 0; int_i <= src_mix.num_breakpoints; int_i++)
        max_index =
            FN_MAX(max_index, gsl_matrix_int_max(src_mix.MIXMAT_param[int_i]));
...
int count_instances(const char *filename, const char *sep_string) {
    ...
    for (int i = input.find("breakpoint", 0); i != string::npos;
         i = input.find("breakpoint", i)) {
        cat_appearances++;
        i++;
    }
...
int max_mm_param_int(const char *filename, const int nbreakpoints) {
    ...
    if (fl_filename == nullptr) {
        DEBUG(DEBUG_ERROR,
              "Error reading " << filename << ": " << strerror(errno))
        exit(2);
    }
...
void input_mixing_matrix_model(mixing_model &l_MIXMAT, const char *infile,
                               const char *in_param_file) {
    ...
    if (age_mixing_matrix == nullptr || age_mixing_matrix_param == nullptr) {
        DEBUG(DEBUG_ERROR, "Error reading " << infile << " or " << in_param_file
                                            << ": " << strerror(errno))
        exit(2);
    }
    ...
    for (int i = 0; i <= l_MIXMAT.num_breakpoints; i++) {
        ...
        gsl_matrix_int_fscanf(age_mixing_matrix_param,
                              l_MIXMAT.MIXMAT_param[i]);
        if (i != l_MIXMAT.num_breakpoints) {
            fscanf(age_mixing_matrix, "%s", temp_string);
            fscanf(age_mixing_matrix, "%d",
                   gsl_vector_int_ptr(l_MIXMAT.breakpoints, i));
            fscanf(age_mixing_matrix_param, "%s %d", temp_string, &temp_int);
        }

        // CALCULATE DOMINANT EIGENVALUE AND CALCULATE AND STORE DOMINANT
        // EIGENVECTOR
        gsl_compute_max_evalue_evector2(
            gsl_vector_ptr(l_MIXMAT.eval_dominant, i),
            l_MIXMAT.evector_MIXMAT_normalised[i], l_MIXMAT.MIXMAT[i]);
    }
    ...
    for (inti = 0; inti <= l_MIXMOD.num_breakpoints; inti++) {
        ...
        for (intj = 0; intj < matrix_i->size1; intj++)
            for (intk = 0; intk < matrix_i->size2; intk++)
                gsl_matrix_set(
                    matrix_i, intj, intk,
                    gsl_matrix_get(l_MIXMOD.MIXMAT[inti], intj, intk) *
                        gsl_vector_get(
                            l_MIXMOD.scalants,
                            gsl_matrix_int_get(l_MIXMOD.MIXMAT_param[inti],
                                               intj, intk)));
    }
}
// MIXING_MATRIX_NORMALIZATION: SCALES THE TIME T MIXING MATRIX BY THE DOMINANT
// EIGENVALUE OF THE POPULATION SCALED MIXING MATRIX AT TIME 0
    ...
    const bool evec_flag = true) // POPULATION PER AGE GROUP
{
    ...
    register gsl_matrix *MIXMAT_dummy =
        gsl_matrix_alloc(l_MIXMOD_ADJUSTED2.MIXMAT[0]->size1,
                         l_MIXMOD_ADJUSTED2.MIXMAT[0]->size2);
    ...
    for (register int i = 0; i <= l_MIXMOD_ADJUSTED2.num_breakpoints; i++) {
        ...
        if (i == 0) { // TIME-0 NEXT GENERATION MATRIX
            gsl_matrix_memcpy(MIXMAT_dummy,
                              l_MIXMOD_ADJUSTED2.MIXMAT_scaled[i]);

            for (register int b = 0; b < l_MIXMOD_ADJUSTED2.MIXMAT[i]->size2;
                 ++b) {
                gsl_vector_view MIXMAT_dummy_col =
                    gsl_matrix_column(MIXMAT_dummy, b);
                gsl_vector_mul(&MIXMAT_dummy_col.vector, l_populationAG);
            }

            // now get dominant eigenvalue and eigenvector of these matrices
            gsl_compute_max_evalue_evector2(
                gsl_vector_ptr(l_MIXMOD_ADJUSTED2.eval_dominant, i),
                l_MIXMOD_ADJUSTED2.evector_MIXMAT_normalised[i], MIXMAT_dummy,
                evec_flag);
        }
        ...
        gsl_matrix_scale(
            l_MIXMOD_ADJUSTED2.MIXMAT_scaled[i],
            1.0 / gsl_vector_get(l_MIXMOD_ADJUSTED2.eval_dominant, 0));
    }
...
int mix_timecat(const int time_steps, const gsl_vector_int *breakpoints,
                const double time_steps_per_day) {

    int out_cat = 0;

    for (; out_cat < breakpoints->size;) {

        if (time_steps_per_day *
                ((double)gsl_vector_int_get(breakpoints, out_cat)) <=
            (double)time_steps) {
            out_cat++;
        } else {
            break;
