...
void gsl_vector_sscanf(const string in_string, gsl_vector *v,
                       const char *chr_delim) {

    int indx = 0;
    for (int i = 0; i < v->size; i++)
        gsl_vector_set(
            v, i, read_from_delim_string<double>(in_string, chr_delim, indx));
}

void gsl_vector_int_sscanf(const string in_string, gsl_vector_int *v,
                           const char *chr_delim) {

    int indx = 0;
    for (int i = 0; i < v->size; i++)
        gsl_vector_int_set(
            v, i, read_from_delim_string<int>(in_string, chr_delim, indx));
}

void gsl_vector_subvector_memcpy(gsl_vector *v_out, gsl_vector *v_in,
                                 const size_t offset) {
    gsl_vector_view v_in_sub = gsl_vector_subvector(v_in, offset, v_out->size);
    gsl_vector_memcpy(v_out, &(v_in_sub.vector));
}

void gsl_vector_int_set_seq(gsl_vector_int *v, const int i_from,
                            const int i_by) {
    for (int i = 0; i < v->size; i++)
        gsl_vector_int_set(v, i, i_from + (i * i_by));
}

void gsl_vector_set_seq(gsl_vector *v, const double dbl_from,
                        const double dbl_by) {
    for (int i = 0; i < v->size; i++)
        gsl_vector_set(v, i, dbl_from + (i * dbl_by));
}

// GSL_DOUBLE_PRODUCT_OF_VECTOR_ELEMENTS RETURNS THE PRODUCT OF THE ELEMENTS OF
// A VECTOR
...
gsl_complex gsl_absmax_vector_complex(const gsl_vector_complex *cplx_vec) {
    double abs_temp, abs_max = 0;
    int i_max;
    for (int i = 0; i < cplx_vec->size; i++) {
        abs_temp = gsl_complex_abs2(gsl_vector_complex_get(cplx_vec, i));
        if (abs_temp > abs_max) {
            abs_max = abs_temp;
            i_max = i;
        }
    }
    return gsl_vector_complex_get(cplx_vec, i_max);
}
