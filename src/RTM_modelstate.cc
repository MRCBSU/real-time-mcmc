...
model_state::model_state(const unsigned int iSize) :
  S(iSize), E_1(iSize), E_2(iSize), I_1(iSize), I_2(iSize), R_pos(iSize), R_neg(iSize), W(iSize), p_lambda(iSize)
{}
model_state::model_state(const model_state& oC) : 
  S(oC.S), E_1(oC.E_1), E_2(oC.E_2), I_1(oC.I_1), I_2(oC.I_2),
  R_pos(oC.R_pos), R_neg(oC.R_neg), W(oC.W), p_lambda(oC.p_lambda)
{}
...
void model_state::fill(const gsl_vector* inS, const gsl_vector* inE_1, const gsl_vector* inE_2, const gsl_vector* inI_1, const gsl_vector* inI_2, const gsl_vector* inR_pos, const gsl_vector* inR_neg, const gsl_vector* inW, const gsl_vector* inp_lambda)
{
