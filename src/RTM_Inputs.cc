...
#define ERROR_FILE_EXIT(err_string, ptr_filename)                              \
    printf(err_string, ptr_filename);                                          \
    exit(2);

// SWAPS DEFAULT VARIABLE DEFAULT VALUES (VARIABLE NAMES
// READ IN AS A DELIMITED STRING) WITH VALUES READ IN FROM FILE
void read_variable_value(const string str_varnames, string &str_var_values,
    ...
    if (tempfile == NULL) {
        DEBUG(DEBUG_WARNING, "Cannot open " << str_filename
    ...
    // CHECK THERE ARE THE SAME NUMBER OF INSTANCES OF THE DELIMITER IN
    // str_var_values
    if (num_instances != count_instances_in_string(str_var_values, ":")) {
        ERROR_FILE_EXIT(
            "Variable names and default values don't match for file %s\n",
            str_filename);
    } else if (num_instances == 0) {
        ERROR_FILE_EXIT(
            "No delimiter detected when searching for variable to find in %s\n",
            str_filename);
    }
    ...
    for (; i_var_end != string::npos;) {

        i_var_end = str_varnames.find(":", i_var_start);
        i_val_end = str_var_values.find(":", i_val_start);

        i_var_mark =
            (i_var_end == string::npos) ? str_varnames.length() : i_var_end;
        i_val_mark =
            (i_val_end == string::npos) ? str_var_values.length() : i_val_end;

        // GET A POINTER TO A NULL-TERMINATED STRING FROM str_varnames
        // str_var NEEDS TO BE PRE-ALLOCATED
        str_var =
            TrimStr(str_varnames.substr(i_var_start, i_var_mark - i_var_start));

        // SEARCH THE FILE FOR A REPLACEMENT VALUE -
        // DO NOTHING IF ONE NOT FOUND, ONLY NEED TO INCREMEMNT i_val_start AND
        // i_val_end
        if (count_instances_in_file(str_filename, str_var.c_str()) > 0) {
            ...
            str_var_values.replace(i_val_start, i_val_mark - i_val_start,
                                   str_replace);
...
#define READ_NEXT_VARIABLE_VALUE                                               \
    read_from_delim_string<int>(str_vardefaults, ":", int_delim_position);
#define READ_NEXT_DOUBLE_VARIABLE_VALUE                                        \
    read_from_delim_string<double>(str_vardefaults, ":", int_delim_position);

void read_mcmc_parameters(register mcmcPars &mcmc_pars, const char *source_file,
                          const string str_varnames, string &str_vardefaults) {
...
void read_global_fixed_parameters(
    register global_model_instance_parameters &fixed_pars,
    const char *source_file, const string str_varnames,
    ...
    fixed_pars.l_tk = (transmission_kernel)
        READ_NEXT_VARIABLE_VALUE; // 0 FOR REED-FROST, 1 FOR MASS ACTION
    ...
    if (fixed_pars.l_GP_likelihood.upper >
    ...
    if (fixed_pars.l_Hosp_likelihood.upper >
        fixed_pars.l_duration_of_runs_in_days)
        printf("Hospitalisations likelihood interval greater than number of "
               "days\n");
    ...
    if (fixed_pars.l_Deaths_likelihood.upper >
        fixed_pars.l_duration_of_runs_in_days)
    ...
    if (fixed_pars.l_Sero_likelihood.upper >
        fixed_pars.l_duration_of_runs_in_days)
        printf("Serological data likelihood interval greater than number of "
               "days\n");
    ...
    R_gl_fac_sublevel(fixed_pars.d_week_numbers_by_day, fixed_pars.l_start_week,
                      fixed_pars.l_start_day_of_week, 7,
                      fixed_pars.l_duration_of_runs_in_days);

    //  if(fixed_pars.l_Viro_likelihood.upper >
    //  gsl_vector_int_max(fixed_pars.d_week_numbers_by_day) &&
    //  fixed_pars.l_Viro_likelihood.lower <
    //  gsl_vector_int_min(fixed_pars.d_week_numbers_by_day)) // weekly data
    if (fixed_pars.l_Viro_likelihood.upper >
            fixed_pars.l_duration_of_runs_in_days &&
        fixed_pars.l_Viro_likelihood.upper <
            fixed_pars.l_Viro_likelihood.lower) // daily data
        printf("Invalid bounds for weekly data to be included in the "
               "virological data likelihood\n");
}

/*
 * Read a form of `<key> = <value>;` from `var_string`. Put `<value>` into
 * `out_string`. Return true on success or false on failure.
 */
bool read_string_from_instruct(string &out_string, const string &var_name,
    ...
    if (++var_dec_start < var_dec_end) {
        out_string = TrimStr(
            var_string.substr(var_dec_start, var_dec_end - var_dec_start));
        return true;
    }
    return false;
}

double read_double(const string var_name, const string var_string,
                   const double default_value = 0.0) {
    string var_substring;
    if (read_string_from_instruct(var_substring, var_name, var_string)) {
        DEBUG(DEBUG_ALL, "No double found for " << var_name
...
int read_int(const string var_name, const string var_string,
             const int default_value = 0) {
    string var_substring;
    if (read_string_from_instruct(var_substring, var_name, var_string)) {
        return atoi(var_substring.c_str());
    } else {
        DEBUG(DEBUG_ALL, "No int found for " << var_name
                                             << " returning default value of"
                                             << default_value)
        return default_value;
    }
...
void read_gsl_vector(gsl_vector *out_gslvec, const string var_name,
                     const string var_string) {
    string var_var_string;
    if (!read_string_from_instruct(var_var_string, var_name, var_string)) {
        DEBUG(DEBUG_ERROR,
              "No value found for " << var_name << " in " << var_string);
        exit(2);
    }
    gsl_vector_sscanf(var_var_string, out_gslvec);
}

void read_gsl_vector_int(gsl_vector_int *out_gslvec_int, const string var_name,
                         const string var_string) {
    string var_var_string;
    if (!read_string_from_instruct(var_var_string, var_name, var_string)) {
        DEBUG(DEBUG_ERROR,
...
void read_gsl_matrix(gsl_matrix *out_gslmat, const string var_name,
                     const string var_string) {
    string var_var_string;
    if (!read_string_from_instruct(var_var_string, var_name, var_string)) {
        DEBUG(DEBUG_ERROR,
              "Could not read " << var_name << " from string " << var_string);
        exit(2);
    }
    gsl_matrix_sscanf(var_var_string, out_gslmat);
}

void read_gsl_matrix_int(gsl_matrix_int *out_gslmat_int, const string var_name,
                         const string var_string) {
    string var_var_string;
    if (!read_string_from_instruct(var_var_string, var_name, var_string)) {
        DEBUG(DEBUG_ERROR,
...
void read_string_array(string *out_strarray, const int n, const string var_name,
    ...
    int indx = 0;
    for (int i = 0; i < n; i++)
        out_strarray[i].assign(prepend + read_from_delim_string<string>(
                                             var_var_string, ",;", indx));
}

int flag_int_or_string(
    const string var_name, const string var_string,
    int default_return = 0) { // INDICATES WHETHER A NAME OR A VECTOR OF VALUES
                              // IS TO BE READ IN FROM THE INPUT var_string
    string var_var_string;
    if (!read_string_from_instruct(var_var_string, var_name, var_string)) {
...
int cut_out_parameter(string &out_substring, const string file_content,
                      const char *param_name) {

    // TAKES A SUBSTRING OF FILE STRING CONTENTS OF ALL INFORMATION RELEVANT TO
    // PARAMETER param_name RETURNS A POSITIVE INTEGER IF STRING param_name IS
    // FOUND IN file_content, ZERO OTHERWISE
    int num_strings = count_instances_in_string(file_content, param_name);
    if (num_strings > 0) {
        int substring_position =
            nth_instance_in_string(file_content, param_name, num_strings);
        ...
        if (var_dec_start == string::npos &&
            var_dec_end == string::npos) { // ERROR
            printf("Error defining variable %s\n", param_name);
            exit(2);
        }
        out_substring =
            file_content.substr(var_dec_start + 1, var_dec_end - var_dec_start);
    }
...
void alloc_and_set_breakpoint_vector(gsl_vector_int *&vector_of_breakpoints,
    ...
    if (temp_string.compare("false") != 0 &&
        ((temp_string.compare("true") == 0) || (flag_alternate_variation))) {
        // if there are breakpoints, then we should have a true value or a
        // numeric vector
        if (temp_string.compare("true") == 0) {
            ...
            vector_of_breakpoints = gsl_vector_int_alloc(
                count_delims_in_string(temp_string, ",") + 1);
...
void read_param_regression(regression_def &reg_def, const int param_dimension,
                           const string str_source, const int num_regions,
                           const int num_days, const int num_ages,
                           const string basedir) {

    int dim_r = 1, dim_t = 1, dim_a = 1;
    // IF THERE IS NO SPATIAL, REGIONAL OR TEMPORAL VARIATION THEN THIS CAN BE
    // "SET TO NULL"

    // A MODEL EXISTS IF ANY OF regional_breakpoints, time_breakpoints or
    // age_breakpoints ARE SPECIFIED
    ...
    reg_def.region_breakpoints = reg_def.time_breakpoints =
        reg_def.age_breakpoints = 0;
    reg_def.design_matrix = 0;

    if (((age_pos != string::npos) || (time_pos != string::npos)) ||
        (reg_pos != string::npos)) {
        // WE HAVE SOME FORM OF VARIATION
        int link_pos = str_source.find("regression_link");
        if (link_pos != string::npos)
            reg_def.regression_link =
                (link_function)read_int("regression_link", str_source);
        else
            reg_def.regression_link = cIDENTITY;

        alloc_and_set_breakpoint_vector(reg_def.region_breakpoints, dim_r,
                                        str_source, "region_breakpoints",
                                        reg_pos, num_regions, false);

        alloc_and_set_breakpoint_vector(reg_def.time_breakpoints, dim_t,
                                        str_source, "time_breakpoints",
                                        time_pos, num_days, true);

        alloc_and_set_breakpoint_vector(reg_def.age_breakpoints, dim_a,
                                        str_source, "age_breakpoints", age_pos,
                                        num_ages, true);

        // THE DESIGN MATRIX...
        // SHOULD HAVE NUMBER OF COLUMNS EQUAL TO THE PARAMETER DIMENSION
        // SHOULD HAVE NUMBER OR ROWS EQUAL TO THE TOTAL NUMBER OF (region,
        // time, age) GROUP COMBINATIONS
        reg_def.design_matrix =
            gsl_matrix_alloc(dim_r * dim_t * dim_a, param_dimension);

        // TO ALLOCATE THE MATRIX, EITHER A FILENAME SHOULD BE GIVEN OR A VECTOR
        // OF NUMBERS
        bool flag_infile =
            (bool)flag_int_or_string("regression_design", str_source);
        if (flag_infile) {
            string str_filename;
            if (!read_string_from_instruct(str_filename, "regression_design",
                                           str_source)) {
                DEBUG(DEBUG_ERROR,
                      "Could not read regression_design from string "
                          << str_source);
                exit(2);
            }
            ...
            if (design_file == nullptr) {
                DEBUG(DEBUG_ERROR, "Error reading " << str_filename << ": "
                                                    << strerror(errno))
                exit(2);
            }
        ...
        } else if (str_source.find("regression_design") !=
                   string::npos) // READ IN THE MATRIX AS A VECTOR
            read_gsl_matrix(reg_def.design_matrix, "regression_design",
                            str_source);

        else // DEFAULT, SET THE MATRIX TO THE IDENTITY (EVEN IF A RECTANGULAR
             // MATRIX)
...
void read_modpar(updateable_model_parameter &modpar, const char *param_name,
                 const string in_string, const double default_value,
                 const string likelihood_flags, const int num_regions,
                 const int num_days, const int num_ages, const string basedir) {
    ...
    /// DOES THE PARAMETER STRING OCCUR IN THE INPUT FILE? IF SO, PUT INTO
    /// VAR_STRING THE RELEVANT PART OF THE FILE
    int num_strings = cut_out_parameter(var_string, in_string, param_name);

    if (num_strings > 0) { // THE STRING IS FOUND

        /// GET PARAMETER DIMENSION
        if (!read_string_from_instruct(var_var_string, "param_value",
                                       var_string)) {
            DEBUG(DEBUG_ERROR,
                  "No value found for param_value in " << var_string);
            exit(2);
        }
        ...
        /// CAN NOW ASSIGN SOME OF THE VECTORS IN THE UPDATEABLE_PARAMETER
        /// STRUCTURE
        ...
        read_gsl_vector_int(modpar.prior_distribution, "prior_distribution",
                            var_string);

        // IF ONE COMPONENT OF prior_distribution IS A MULTIVARIATE DISTRIBUTION
        // THEN ALL MUST BE
        ...
        modpar.flag_update =
            (gsl_vector_int_max(modpar.prior_distribution) > (int)cCONSTANT);

        /// ALLOCATE AND ASSIGN ALL PROPERTIES WHOSE USE IS CONDITIONAL ON BEING
        /// A STOCHASTIC NODE IN THE MODEL
        if (modpar.flag_update) {
            ...
            modpar.prior_params =
                (gsl_vector **)calloc(var_dimension, sizeof(gsl_vector *));

            // DETERMINE WHETHER OR NOT THE PARAMETER HAS A HYPERPRIOR (it will
            // have the property prior_hyper set to a value > 0 if so
            string tempstring;
            modpar.flag_hyperprior = read_string_from_instruct(
                tempstring, "prior_hyper", var_string);
            modpar.flag_hyperprior =
                modpar.flag_hyperprior && atoi(tempstring.c_str()) > 0;

            // ERROR IF HYPERPRIOR IS SPECIFIED FOR A MULTIVARIATE PARAMETER
            if (modpar.flag_hyperprior &&
                (gsl_vector_int_get(modpar.prior_distribution, 0) ==
                 (int)cMVNORMAL)) {
                DEBUG(DEBUG_ERROR,
                      "Multivariate normal selected for the hyperprior "
                          << param_name);
                exit(2);
            }

            // COMPONENTWISE ALLOCATATION OF MEMORY FOR THE PRIORS...
            // HYPERPRIORS SHOULD HAVE THEIR PARAMETERS SET ELSEWHERE
            if (!modpar.flag_hyperprior) {
                DEBUG(DEBUG_ALL, "Has a hyperprior")

                for (int_marker = 0; int_marker < var_dimension;) {
                    int nparam = num_parameters_by_distribution(
                        (distribution_type)gsl_vector_int_get(
                            modpar.prior_distribution, int_marker),
                        var_dimension);
                    modpar.prior_params[int_marker++] =
                        (nparam > 0) ? gsl_vector_alloc(nparam) : 0;
                    total_number_prior_params +=
                        nparam; // modpar.prior_params[int_marker++]->size;
                }
            }

            // ASSIGN PROPOSAL VARIANCES
            read_gsl_vector(modpar.proposal_variances, "proposal_variance",
                            var_string);

            // CHECK THAT WE'RE NOT DEALING WITH A PARAMETER WHICH HAS A
            // HYPERPRIOR - PRIOR LINKS WILL BE ESTABLISHED ELSEWHERE
            if (!modpar.flag_hyperprior) {
                gsl_vector *temp_vec =
                    gsl_vector_alloc(total_number_prior_params);

                read_gsl_vector(temp_vec, "prior_parameters", var_string);

                int offset = 0;
                for (int_marker = 0; int_marker < var_dimension; int_marker++) {
                    if (modpar.prior_params[int_marker] != 0) {
                        gsl_vector_subvector_memcpy(
                            modpar.prior_params[int_marker], temp_vec, offset);
            ...
            modpar.flag_transmission_model =
                atoi((likelihood_flags.substr(0, 1)).c_str());
            modpar.flag_reporting_model =
                atoi((likelihood_flags.substr(1, 1)).c_str());
            modpar.flag_GP_likelihood =
                atoi((likelihood_flags.substr(2, 1)).c_str());
            modpar.flag_Hosp_likelihood =
                atoi((likelihood_flags.substr(3, 1)).c_str());
            modpar.flag_Sero_likelihood =
                atoi((likelihood_flags.substr(4, 1)).c_str());
            modpar.flag_Viro_likelihood =
                atoi((likelihood_flags.substr(5, 1)).c_str());
        }
    ...
    // ESTABLISH THE MAPS FROM THE PARAMETER VALUES TO THE VALUES PASSED TO THE
    // REGIONAL STRUCTURES
    read_param_regression(modpar.map_to_regional, modpar.param_value->size,
                          var_string, num_regions, num_days, num_ages, basedir);
}

/////// INITIALISE PRIOR DENSITIES
void initialise_prior_density(
    updateable_model_parameter &modpar,
    double (*prior_density_fn)(const double &, const distribution_type &,
                               const gsl_vector *)) {

    gsl_vector_int *prior_distributions =
        gsl_vector_int_alloc(modpar.param_value->size);
    ...
    if (gsl_vector_int_get(prior_distributions, 0) != (int)cMVNORMAL) {
        for (int int_marker = 0; int_marker < modpar.param_value->size;
             int_marker++)
            if (modpar.prior_params[int_marker] != 0)
                modpar.log_prior_dens += prior_density_fn(
                    gsl_vector_get(modpar.param_value, int_marker),
                    (distribution_type)gsl_vector_int_get(prior_distributions,
                                                          int_marker),
                    modpar.prior_params[int_marker]);
    } else {
        // SET UP THE MULTIVARIATE NORMAL MEAN AND COVARIANCE STRUCTURES
        gsl_vector *mv_mean = gsl_vector_alloc(modpar.param_value->size);
        gsl_matrix *mv_covar = gsl_matrix_alloc(modpar.param_value->size,
                                                modpar.param_value->size);

        for (int int_marker = 0; int_marker < modpar.param_value->size;
             int_marker++) {
            gsl_vector_set(mv_mean, int_marker,
                           gsl_vector_get(modpar.prior_params[int_marker], 0));
            gsl_vector_view mv_covar_subrow =
                gsl_vector_subvector(modpar.prior_params[int_marker], 1,
                                     modpar.prior_params[int_marker]->size - 1);
            gsl_matrix_set_row(mv_covar, int_marker, &mv_covar_subrow.vector);
        }

        // TRANSFER THIS DATA INTO THE MVNORM CLASS HELD IN THE
        // UPDATEABLE_MODEL_PARAMETERS STRUCTURE
        modpar.prior_multivariate_norm = new mvnorm(mv_mean, mv_covar);

        // EVALUATE THE LOG-DENSITY
        if (modpar.flag_hyperprior)
            modpar.log_prior_dens =
                (*modpar.prior_multivariate_norm).ld_mvnorm(modpar.param_value);
        else
            modpar.log_prior_dens = (*modpar.prior_multivariate_norm)
                                        .ld_mvnorm_nonnorm(modpar.param_value);
...
void node_links(globalModelParams &in_pars, const string str_param_input) {
    ...
    for (inti = 0; inti < in_pars.size_param_list; inti++) {
        ...
        for (intj = 0; intj < in_pars.size_param_list; intj++) {
            ...
            if (inti != intj) {
                ...
                // NO PARAMETERS ARE HYPERPARAMETERS BY DEFAULT, SO NEEDS TO BE
                // SPECIFIED IN FILE. CHECK THE VARIABLE APPEARS IN THE FILE
                // STORE THE VARIABLE STRING
                int num_strings = cut_out_parameter(
                    temp_param, str_param_input,
                    in_pars.param_list[intj].param_name.c_str());

                if (num_strings > 0) {

                    // FIND THE PRIOR_PARAMS VALUE, they are expected to be
                    // missing sometimes
                    (void)!read_string_from_instruct(
                        temp_prior_param, "prior_parameters", temp_param);
                    ...
                    temp_prior_param.erase(
                        FN_MAX(temp_prior_param.length(), 1) - 1, 1);

                    if (temp_prior_param.compare(
                            in_pars.param_list[inti].param_name) == 0) {
                        // PARAMETER j IS A CHILD NODE OF PARAMETER i
                        // SET PRIOR_PARAMS OF j TO BE A POINTER TO PARAM_VALUES
                        // OF i
                        for (int intk = 0;
                             intk < in_pars.param_list[intj].param_value->size;
                             intk++) {
                            if (gsl_vector_int_get(
                                    in_pars.param_list[intj].prior_distribution,
                                    1) != (int)cCONSTANT)
                                in_pars.param_list[intj].prior_params[intk] =
                                    in_pars.param_list[inti].param_value;
                        }
...
void aggregate_delays(infection_to_data_delay &itdd) {
    ...
    for (int inti = 0; inti < itdd.num_components; inti++) {
        ...
        itdd.overall_delay.gamma_sd +=
            gsl_pow_2(itdd.component_delays[inti]
                          .gamma_sd); // FOR THE MOMENT, THE GAMMA_SD IS STORING
                                      // THE AGGREGATE VARIANCE
    }
    itdd.overall_delay.gamma_shape =
        gsl_pow_2(itdd.overall_delay.gamma_mean) / itdd.overall_delay.gamma_sd;
    itdd.overall_delay.gamma_rate =
        itdd.overall_delay.gamma_mean / itdd.overall_delay.gamma_sd;
    itdd.overall_delay.gamma_scale = 1.0 / itdd.overall_delay.gamma_rate;
    itdd.overall_delay.gamma_sd = sqrt(
        itdd.overall_delay.gamma_sd); // NOW WE'VE USED THE VARIANCE, TRANSFORM
                                      // BACK TO THE STANDARD DEVIATION
}

void calculate_fixed_distribution_function(infection_to_data_delay &itdd,
                                           double delta_t = 1) {

    // FIND THE MAXIMAL INTEGER NECESSARY TO ADEQUATELY TRUNCATE THE DELAY
    // DISTRIBUTIONS
    double dbl_k = gsl_cdf_gamma_Pinv(
        1 - EPSILON_TOLERANCE, itdd.overall_delay.gamma_shape,
        itdd.overall_delay.gamma_scale); // MAXIMAL NUMBER OF DAYS TO CONSIDER
    int int_k0 = (int)ceil(dbl_k / delta_t);
    double cumulative_probability = 0.0;

    itdd.distribution_function = gsl_vector_alloc(
        ++int_k0); // INCREMENT int_k0 TO ENSURE THAT THE FINAL
                   // PROBABILITY IS LESS THAN EPSILON_TOLERANCE

    for (int int_i = 0; int_i < (int_k0 - 1); int_i++) {

        gsl_vector_set(itdd.distribution_function, int_i,
                       gsl_cdf_gamma_P(((double)int_i + 1) * delta_t,
                                       itdd.overall_delay.gamma_shape,
                                       itdd.overall_delay.gamma_scale) -
                           cumulative_probability);

        cumulative_probability +=
            gsl_vector_get(itdd.distribution_function, int_i);
    }

    gsl_vector_set(
        itdd.distribution_function, int_k0 - 1,
        1.0 - cumulative_probability); // TRUNCATE THE DISTRIBUTION AT
                                       // GREATER THAN (int_k0 - 1) DAYS
}

void load_delays(st_delay &out_delay, const string str_name, const double mean,
                 const double sd, const string input_string) {
    ...
    int num_strings =
        cut_out_parameter(var_string, input_string, str_name.c_str());
...
void read_global_model_parameters(
    globalModelParams &in_pars, const char *source_file,
    const string str_var_names, const string &str_var_initvals,
    const string &str_var_likflags, const string &str_delay_names,
    const string &str_delay_means, const string &str_delay_sds,
    const string &str_delay_flags, const int num_regions, const int num_days,
    const int num_ages, const double reporting_time_steps_per_day) {

    string str_source;
    int int_delim_name_position = 0, int_delim_val_position = 0,
        int_delim_flag_position = 0;
    int inti;
    ...
    if ((num_instances != count_instances_in_string(str_var_initvals, ":")) ||
        ...
        printf("Unequal length of input strings to fn: "
               "read_global_model_parameters\n");
        exit(2);
    }
    ...
    const string basedir = std::experimental::filesystem::path(source_file)
                               .parent_path()
                               .string() +
                           "/";
    ...
    for (inti = 0; inti < num_instances; inti++) {
        ...
        string str_param_name = read_from_delim_string<string>(
            str_var_names, ":", int_delim_name_position);

        // GET VARIABLE DEFAULT FROM DEFAULT STRING
        double dbl_param_val = read_from_delim_string<double>(
            str_var_initvals, ":", int_delim_val_position);

        // GET VARIABLE LIKELIHOOD INDICATORS FROM DEFAULT STRING
        string str_param_flags = read_from_delim_string<string>(
            str_var_likflags, ":", int_delim_flag_position);

        // POPULATE THE STRUCTURE
        read_modpar(in_pars.param_list[inti], str_param_name.c_str(),
                    str_source, dbl_param_val, str_param_flags, num_regions,
                    num_days, num_ages, basedir);
    }
    ...
    for (inti = 0; inti < num_instances; inti++)
        // CALCULATE INITIAL PRIOR DENSITY - BUT FIRST CHECK THAT THE NODE IS
        // STOCHASTIC
        if (in_pars.param_list[inti].flag_update) {
            if (in_pars.param_list[inti].flag_hyperprior)
                initialise_prior_density(in_pars.param_list[inti],
                                         R_univariate_prior_log_density);
            else
                initialise_prior_density(
                    in_pars.param_list[inti],
                    R_univariate_prior_log_density_nonnorm);
        }
    ...
    if ((num_instances != count_instances_in_string(str_delay_means, ":")) ||
        ...
        printf("Unequal length of default delay strings to fn: "
               "read_global_model_parameters\n");
        exit(2);
    }
    ...
    for (inti = 0; inti < num_instances; inti++) {
        ...
        // get the name
        string str_param_name = read_from_delim_string<string>(
            str_delay_names, ":", int_delim_name_position);

        // get the gamma mean
        double dbl_param_mean = read_from_delim_string<double>(
            str_delay_means, ":", int_delim_mean_position);

        // get the gamma sd
        double dbl_param_sd = read_from_delim_string<double>(
            str_delay_sds, ":", int_delim_sd_position);

        // get the delay flags
        str_param_flags[inti] = read_from_delim_string<string>(
            str_delay_flags, ":", int_delim_flag_position);

        // load the individual delays into an element of temp_delay
        load_delays(temp_delay[inti], str_param_name, dbl_param_mean,
                    dbl_param_sd, str_source);

        gp_delay_counter += atoi((str_param_flags[inti].substr(0, 1)).c_str());
        hosp_delay_counter +=
            atoi((str_param_flags[inti].substr(1, 1)).c_str());
        death_delay_counter +=
            atoi((str_param_flags[inti].substr(2, 1)).c_str());
    }
    ...
    for (inti = gp_delay_counter = hosp_delay_counter = death_delay_counter = 0;
         inti < num_instances; inti++) {
        if ((bool)atoi((str_param_flags[inti].substr(0, 1)).c_str())) {
            st_delay_memcpy(in_pars.gp_delay.component_delays[gp_delay_counter],
                            temp_delay[inti]);
            gp_delay_counter++;
        }
        if ((bool)atoi((str_param_flags[inti].substr(1, 1)).c_str())) {
            st_delay_memcpy(
                in_pars.hosp_delay.component_delays[hosp_delay_counter],
                temp_delay[inti]);
            hosp_delay_counter++;
        }
        if ((bool)atoi((str_param_flags[inti].substr(2, 1)).c_str())) {
            st_delay_memcpy(
                in_pars.death_delay.component_delays[death_delay_counter],
                temp_delay[inti]);
            death_delay_counter++;
        }
    }
    ...
    calculate_fixed_distribution_function(in_pars.gp_delay,
                                          1.0 / reporting_time_steps_per_day);
    calculate_fixed_distribution_function(in_pars.hosp_delay,
                                          1.0 / reporting_time_steps_per_day);
    calculate_fixed_distribution_function(in_pars.death_delay,
                                          1.0 / reporting_time_steps_per_day);
}
...
void data_matrices_fscanf(
    const string &infilestring, gsl_matrix *reported_data, const int col_skip,
    const int row_skip) { // NEEDS SOME ERROR HANDLING IN HERE.
    register int inti, intj;
    ...
    if (infile == nullptr) {
        DEBUG(DEBUG_ERROR,
              "Could not read " << infilestring << ": " << strerror(errno));
        exit(2);
    }
    ...
    for (inti = 0; inti < (reported_data->size1 + row_skip); inti++) {
        ...
        for (intj = 0; intj < (reported_data->size2 + col_skip); intj++) {
            ...
            if ((inti < row_skip) || (intj < col_skip)) {
                ...
                matches = fscanf(infile, "%lf",
                                 gsl_matrix_ptr(reported_data, inti - row_skip,
                                                intj - col_skip));
            }
            ...
            if (matches == EOF) {
                DEBUG(DEBUG_ERROR,
                      "Error reading from "
                          << infilestring << " at position (" << inti << ", "
                          << intj << "), possibly fewer than "
...
/// READ IN THE DATA MATRICES AS FORMATTED BY THE HPA
void data_int_matrices_fscanf(
    string infilestring, gsl_matrix_int *reported_data, const int col_skip = 0,
    const int row_skip = 0) { // NEEDS SOME ERROR HANDLING IN HERE.
    register int inti, intj;
    char str_row_date[25];
    double dbl_dummy;
    ...
    for (inti = 0; inti < (reported_data->size1 + row_skip); inti++) {
        ...
        for (intj = 0; intj < (reported_data->size2 + col_skip); intj++) {
            ...
            if ((inti < row_skip) || (intj < col_skip))
                ...
                fscanf(infile, "%d",
                       gsl_matrix_int_ptr(reported_data, inti - row_skip,
                                          intj - col_skip));
        }
    }
...
void fetch_filenames(string *out_string, const int &num_strings,
    ...
    if (!var_name.empty())
        if (!read_string_from_instruct(var_filenames, var_name, str_file)) {
            ...
            // var_name.c_str());
            for (int int_i = 0;
                 int_i < num_strings;) // THIS CATERS FOR THE CASE OF NO
                                       // MATCHING STRING.. WHAT IF THERE ARE
                                       // INSUFFICIENT FILENAMES SPECIFIED? NEED
...
void read_filenames_and_data_matrices(
    ...
    const string var_string, const string base_dir, const int col_skip = 0,
    ...
    for (int int_i = 0; int_i < num_regions; int_i++)
        ...
        data_matrices_fscanf(str_vec_filenames[int_i], data_matrices[int_i],
                             col_skip, row_skip);
...
// READ THE FILENAMES TO A TEMP VECTOR
void read_filenames_and_data_int_matrices(
    gsl_matrix_int **data_matrices, const int num_regions,
    const string var_name, const string var_string, const string base_dir,
    const int col_skip = 0, const int row_skip = 0) {
    ...
    for (int int_i = 0; int_i < num_regions; int_i++)
        ...
        data_int_matrices_fscanf(str_vec_filenames[int_i], data_matrices[int_i],
                                 col_skip, row_skip);
...
void read_metaregion_datatype(rtmData **obj_data, const gsl_matrix *population,
                              string *countfiles, string *denomfiles,
                              const string base_dir, const int &num_regions,
                              const string str_var_count,
                              const string str_var_denom,
                              const string str_var_agg,
                              const string &str_source,
                              const bool &normalise_flag) {
    ...
    for (int int_i = 0; int_i < num_regions; int_i++) {
        obj_data[int_i]->setDim(missing_data, str_source, str_var_agg,
                                NUM_AGE_GROUPS);
        obj_data[int_i]->read(denomfiles[int_i], countfiles[int_i]);
        if (normalise_flag) {
            gsl_vector_const_view population_row =
                gsl_matrix_const_row(population, int_i);
...
void read_data_inputs(Region *meta_region, const string str_input_filename,
    ...
    if (num_strings == 0) {
        ERROR_FILE_EXIT("Data structure not found in file %s",
                        str_input_filename.c_str());
    }
    ...
    for (int_i = 0; int_i < num_regions; int_i++) {
        ...
        meta_region[int_i].total_population =
            gsl_vector_sum_elements(meta_region[int_i].population);
    }

    // READ IN THE DATA..
    // CYCLING THROUGH DATA STREAMS, POPULATE THE DATA OBJECTS WITHIN EACH
    // REGION.
    ...
    if (meta_region->GP_data != 0) {
        ...
        read_metaregion_datatype(
            meta_data_type, tempmat, countfiles, denomfiles, base_dir,
            num_regions, "regions_gp_count_data", "regions_gp_coverage_data",
            "regions_gp_aggregation", str_var, cTRUE);
        for (int_i = 0; int_i < num_regions;
             int_i++) // CHECK the actions of the following:
            gsl_matrix_realloc(
                meta_region[int_i].det_model_params.l_gp_negbin_overdispersion,
    ...
    if (meta_region->Hospitalisation_data != 0) {
        ...
        read_metaregion_datatype(meta_data_type, tempmat, countfiles,
                                 denomfiles, base_dir, num_regions,
                                 "regions_hosp_data", "",
        ...
        for (int_i = 0; int_i < num_regions;
            ...
            gsl_matrix_realloc(
                meta_region[int_i]
                    .det_model_params.l_hosp_negbin_overdispersion,
                meta_data_type[int_i]->getDim1(),
                meta_data_type[int_i]->getDim2());
    }
    // IF DEATHS DATA?
...
void read_mixmod_structure_inputs(
    ...
    const string base_dir, const global_model_instance_parameters src_gmp) {
    ...
    int num_strings =
        cut_out_parameter(str_var, str_file_contents, "mixing_model");

    if (num_strings == 0) {
        ERROR_FILE_EXIT("Mixing structure not specified in file %s",
                        str_input_filename.c_str());
    }

    // FIRSTLY NEED TO KNOW HOW MANY TEMPORAL BREAKPOINTS THERE ARE - THIS
    // SHOULD ALLOCATE THE MEMORY TO THE BREAKPOINT COMPONENT.
    alloc_and_set_breakpoint_vector(
        base_mm.breakpoints, base_mm.num_breakpoints, str_var,
        "time_breakpoints", str_var.find("time_breakpoints"),
        src_gmp.l_duration_of_runs_in_days, true);
    ...
    for (int int_i = 0; int_i <= base_mm.num_breakpoints;) {
        base_mm.MIXMAT[int_i] =
            gsl_matrix_alloc(NUM_AGE_GROUPS, NUM_AGE_GROUPS);
        base_mm.MIXMAT_scaled[int_i] =
            gsl_matrix_alloc(NUM_AGE_GROUPS, NUM_AGE_GROUPS);
        base_mm.MIXMAT_param[int_i++] =
            gsl_matrix_int_alloc(NUM_AGE_GROUPS, NUM_AGE_GROUPS);
    }
    ...
    read_filenames_and_data_matrices(base_mm.MIXMAT,
                                     base_mm.num_breakpoints + 1,
                                     "base_matrices", str_var, base_dir);
