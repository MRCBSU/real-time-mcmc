...
void flagclass::flagclass_static_new() {
    if (instances == 0) {
        ...
        for (int int_i = 0; int_i < iSize; int_i++) {
            rmp_member_names[int_i].assign(read_from_delim_string<string>(
                all_names, ",;", int_delim_position));
        }
    }
}
...
string flagclass::UPItoRMP(const int &iUPI_index) {
    ...
    case AIP_INDEX:
        return "l_average_infectious_period : l_R0_init : l_I0 : "
               "l_R0_Amplitude";
...
flagclass::flagclass(const int &iUPI_index) {
    flagclass_static_new(); // allocates static memory and sets static variables
