...
#define ERROR_TEXT(str_err_msg, str_err_file)                                  \
    printf(str_err_msg, str_err_file);                                         \
    exit(2);
...
data_type rtmData::get_likelihood_type() { return likelihood_type; }
...
void rtmData::setDim(const bool &missing_data, const string &str_source,
                     const string &str_property, const int &smax) {
    int dim_s;
    col_grouping.clear();
    ...
    if (!missing_data) {
        ...
        if (!read_string_from_instruct(temp_string, str_property, str_source)) {
            std::cerr << "Could not read " << str_property << " from "
                      << str_source << std::endl;
            exit(2);
        }
...
void rtmData::read(const string &denomfile, const string &countfile) {
    ...
    if (denomfile.compare("") != 0)
        ...
        if (likelihood_type == cBINOMIAL) {
            ...
            gsl_matrix_set_all(
                denoms,
                1.0); // Default value of 1.. everyone is observable.
    }
...
void rtmData::data_population_sizes(const gsl_vector *ptr_vec_popn) {
    ...
    // ptr_vec_popn doesn't necessarily have the requisite length, might need to
    // aggregate over col_grouping
    if (ptr_vec_popn->size != denoms->size2) {
        if (ptr_vec_popn->size > denoms->size2) {
            R_by_sum_gsl_vector_mono_idx(data_population, popn_weights,
                                         ptr_vec_popn, col_grouping);
        } else {
            cout << "Input vector shorter than output vector, cannot contract "
                    "over "
                    "indices."
                 << endl;
            exit(2);
        ...
        gsl_vector_memcpy(data_population, ptr_vec_popn);
}
...
double
rtmData::lfx(gsl_matrix *mat_expected,
             const gsl_matrix *
                 mat_dispersion) { // Note this function will alter the contents
                                   // of mat_expected after scaling by denoms.
    gsl_matrix_view N =
        gsl_matrix_submatrix(denoms, lbounds.lower - 1, 0,
                             lbounds.upper - lbounds.lower + 1, denoms->size2);
    gsl_matrix_view mu = gsl_matrix_submatrix(
        mat_expected, lbounds.lower - 1, 0, lbounds.upper - lbounds.lower + 1,
        mat_expected->size2);
    if (likelihood_type != cBINOMIAL)
        gsl_matrix_mul_elements(&mu.matrix, &N.matrix);
    gsl_matrix_view X =
        gsl_matrix_submatrix(counts, lbounds.lower - 1, 0,
                             lbounds.upper - lbounds.lower + 1, counts->size2);
    switch (likelihood_type) {
    case cPOISSON:
        return fn_log_lik_countdata(&X.matrix, &mu.matrix);
    case cNEGBIN: {
        gsl_matrix_const_view eta = gsl_matrix_const_submatrix(
            mat_dispersion, lbounds.lower - 1, 0,
            lbounds.upper - lbounds.lower + 1, mat_dispersion->size2);
...
int rtmData::getDim1() { return counts->size1; }
int rtmData::getDim2() { return counts->size2; }
const gsl_vector *rtmData::access_weights() const { return popn_weights; }
const vector<int> rtmData::access_groups() const { return col_grouping; }
