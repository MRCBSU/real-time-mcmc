...
mvnorm::mvnorm(const gsl_vector *in_mean, const gsl_matrix *in_covariance) {
    if (in_mean->size != in_covariance->size1 ||
        in_mean->size != in_covariance->size2) {
        printf(
            "Mean vector and covariance matrices not of matching dimension\n");
        exit(2);
    }
    ...
    for (int int_i = 1; int_i < in_covariance->size1; int_i++)
        for (int int_j = 0; int_j < int_i; int_j++) {
            if (gsl_matrix_get(in_covariance, int_i, int_j) !=
                gsl_matrix_get(in_covariance, int_j, int_i)) {
    ...
    covariance_inverse =
        gsl_matrix_alloc(in_covariance->size1, in_covariance->size2);
    gsl_matrix_memcpy(covariance_inverse, in_covariance);
    gsl_linalg_cholesky_decomp(covariance_inverse);
    // The product of diagonal entries of the cholesky decomposition is the
...
double mvnorm::ld_mvnorm_ratio(const gsl_vector *in_x1,
                               const gsl_vector *in_x2) {
