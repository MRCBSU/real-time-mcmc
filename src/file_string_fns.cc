...
#define READ_ERROR_MESSAGE                                                     \
    printf("No instances of string:\"%s\" in file %s. Instance expected\n",    \
           varname, filename);                                                 \
    exit(2);
...
// RETURNS A POINTER, str_ptr_out, TO THE n_instance^TH OCCURENCE OF THE
// CHARACTER ARRAY POINTED TO BY sep_string IN THE FILE NAMED filename MEMORY
// USED BY str_ptr_out IS ALLOCATED AND NEEDS TO BE FREED BY THE CALLING ROUTINE
void nth_instance(char *&str_ptr_out, const char *filename,
                  const char *sep_string, const int n_instance) {
    ...
    if (n_instance > tot_appearances) { // ERROR
        printf("Insufficient occurences of %s in %s\n", sep_string, filename);
        exit(2);
    }

    int substring_position =
        nth_instance_in_string(input, sep_string, tot_appearances);

    if (substring_position > -1) {
        // REALLOC THE MEMORY OF str_ptr_out (OF SIMPLY ALLOC IF NULL)
        if (str_ptr_out == NULL)
            str_ptr_out = (char *)calloc(
                strlen(input.c_str() + substring_position) + 1,
                sizeof(char)); // +1 TO ALLOW FOR THE NULL TERMINATING CHARACTER
        else
            str_ptr_out = (char *)realloc(
                str_ptr_out,
                (strlen(input.c_str() + substring_position) + 1) *
                    sizeof(char)); // +1 TO ALLOW FOR THE NULL TERMINATING
                                   // CHARACTER
...
void read_double_input(double &outval, const char *filename,
                       const char *varname) {
...
void read_char_input(char *out_string, const char *filename,
                     const char *varname, const int max_string_length) {
    ...
    if (tempchar != NULL) {
        ...
        if (strlen(out_string) >= max_string_length) { // ERROR
            printf(
                "Input string exceeds max number of characters (length = %d)\n",
