...
template <typename D>
D read_from_delim_string(const string &str_in, const char *str_delim,
                         int &indx) {
...
template string read_from_delim_string<string>(const string &, const char *,
                                               int &);
template int read_from_delim_string<int>(const string &, const char *, int &);
template double read_from_delim_string<double>(const string &, const char *,
                                               int &);
...
int count_delims_in_string(const string in_string, const char *delims) {
    ...
    for (counter = 0; int_marker != string::npos;)
        int_marker = in_string.find_first_of(delims, int_marker + 1 -
                                                         ((int)counter++ == 0));
    return --counter;
}
...
int count_instances_in_string(const string in_string, const char *sep_string) {
    ...
    for (int i = in_string.find(sep_string, 0); i != string::npos;
        ...
        if ((i == (i_start + 1)) &&
            ((i + strlen(sep_string)) ==
             i_end)) // THEN WE HAVE A MATCH FOR THE WORD
            cat_appearances++;
        i++;
    }
...
int nth_instance_in_string(const string in_string, const char *sep_string,
                           const int n_instance) {

    // RETURNS THE POSITION OF THE N^TH INSTANCE OF SEP_STRING AS A WORD IN THE
    ...
    if (n_instance <= 0) {
        ...
        for (; num_instance < n_instance; ++i) {
            i = in_string.find(sep_string, i);
            i_start =
                (i == 0) ? -1 : in_string.find_first_of(STRING_DELIMS, i - 1);
            i_end =
                in_string.find_first_of(STRING_DELIMS, i + strlen(sep_string));
            if ((i == (i_start + 1)) &&
                ((i + strlen(sep_string)) ==
                 i_end)) // THEN WE HAVE A MATCH FOR THE WORD
                num_instance++;
