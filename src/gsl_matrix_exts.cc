...
void gsl_matrix_set_row_dbl(gsl_matrix *m, const size_t i_row,
                            const double dbl_input) {
...
void gsl_matrix_set_column_dbl(gsl_matrix *m, const size_t i_col,
                               const double dbl_input) {
    gsl_vector_view target_col = gsl_matrix_column(m, i_col);
    gsl_vector_set_all(&(target_col.vector), dbl_input);
}

void gsl_matrix_set_subrow_dbl_memcpy(gsl_matrix *m, const size_t i_row,
                                      const size_t offset, const size_t n,
                                      const gsl_vector *dbl_input) {
    gsl_vector_view target_subrow = gsl_matrix_subrow(m, i_row, offset, n);
    gsl_vector_memcpy(&(target_subrow.vector), dbl_input);
}

void gsl_matrix_set_subcolumn_dbl_memcpy(gsl_matrix *m, const size_t i_col,
                                         const size_t offset, const size_t n,
                                         const gsl_vector *dbl_input) {
    gsl_vector_view target_subcolumn =
        gsl_matrix_subcolumn(m, i_col, offset, n);
    gsl_vector_memcpy(&(target_subcolumn.vector), dbl_input);
}

void gsl_matrix_set_subrow_dbl(gsl_matrix *m, const size_t i_row,
                               const size_t offset, const size_t n,
                               const double dbl_input) {
    register int i;
    if (i_row < 0 || offset < 0 || i_row >= m->size1 ||
        offset - 1 + n >= m->size2)
        perror("Error; trying to assign outside matrix limits in "
               "gsl_matrix_set_subrow_dbl\n");
    if (n < 0)
        perror("Error: trying to assign a negative number of entries in "
               "gsl_matrix_set_subrow_dbl\n");
    for (i = 0; i < n; i++)
        gsl_matrix_set(m, i_row, offset + i, dbl_input);
}

void gsl_matrix_set_subcolumn_dbl(gsl_matrix *m, const size_t j_col,
                                  const size_t offset, const size_t n,
                                  const double dbl_input) {
    register int j;
    if (j_col < 0 || offset < 0 || j_col >= m->size2 ||
        offset - 1 + n >= m->size1)
        perror("Error; trying to assign outside matrix limits in "
               "gsl_matrix_set_subcolumn_dbl\n");
    if (n < 0)
        perror("Error: trying to assign a negative number of entries in "
               "gsl_matrix_set_subcolumn_dbl\n");
    for (j = 0; j < n; j++)
...
// GSL_MATRIX_MULTIPLICATION_VECTOR_DOUBLE; MULTIPLYING A MATRIX WITH A VECTOR
// IN DOUBLE FORMAT
void gsl_matrix_multiplication_vector_dbl(
    ...
    // ERROR IDENTIFYING
    if (A->size2 != b->size)
        perror("Dimension mismatch in gsl_matrix_multiplication_vector_double");
...
void gsl_matrix_multiplication_vector_product_by_row_dbl(gsl_matrix *A,
                                                         const gsl_vector *b) {
    if (A->size2 != b->size)
        perror("Dimension mismatch in "
...
void gsl_compute_max_evalue_evector2(
    double *evalue,      // EIGENVALUE OUTPUT
    gsl_vector *evector, // EIGENVECTOR OUTPUT IS NORMALIZED!
    const gsl_matrix
        *M, // M IS ALWAYS A MATRIX OF DIMENSIONS POP_RANGES X POP_RANGES
    const bool evec_flag) // Do eigenvectors need to be calculated)
{
    if (M->size1 != M->size2) {
        ...
        if (evec_flag) {
            ...
            // gsl_eigen_nonsymmv_sort(eval, m, GSL_EIGEN_SORT_ABS_DESC); //
            // does this function even exist? If not, check the max of a
            // gsl_vector.
        }
        ...
        if (eval_i_im_abs < 1e-08) {
            if (evec_flag) {
                for (int i = 0; i < a; i++)
                    gsl_vector_set(
                        evector, i,
                        GSL_REAL(gsl_matrix_complex_get(evec, i, 0)));
            }
        } else {
            // DOMINANT EIGENVALUE IS COMPLEX.
            // RETURN A VALUE OF THE EIGENVALUE THAT INDICATES AN ERROR. LEAVES
...
void gsl_matrix_sscanf(const string in_string, gsl_matrix *m,
    ...
    for (int i = 0; i < m->size1; i++)
        for (int j = 0; j < m->size2; j++)
            gsl_matrix_set(
                m, i, j,
                read_from_delim_string<double>(in_string, chr_delim, indx));
}

void gsl_matrix_int_sscanf(const string in_string, gsl_matrix_int *m,
                           const char *chr_delim) {
    ...
    for (int i = 0; i < m->size1; i++)
        for (int j = 0; j < m->size2; j++)
            gsl_matrix_int_set(
                m, i, j,
                read_from_delim_string<int>(in_string, chr_delim, indx));
}

// ---- Memory Allocation
void gsl_matrix_realloc(gsl_matrix *&Mat, const int &new_dim1,
                        const int &new_dim2) {
    gsl_matrix_free(Mat);
