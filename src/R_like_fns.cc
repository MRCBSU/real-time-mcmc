				      ...
				      const gsl_vector* params)
{
  ...
  case cCONSTANT :
    return 0;
  case cGAMMA : // FIRST PARAMETER IS THE SHAPE PARAMETER, SECOND PARAMETER IS THE RATE PARAMETER
    return ((gsl_vector_get(params, 0) - 1) * gsl_sf_log(x)) - (gsl_vector_get(params, 1) * x);
  ...
  case cNORMAL : // FIRST PARAMETER IS THE MEAN, SECOND PARAMETER IS THE STANDARD DEVIATION
  case cHALFNORMAL : // AS THE NORMAL DISTRIBUTION, BUT CONSTRAINED TO BE GREATER THAN ZERO
    return gsl_pow_2((x - gsl_vector_get(params, 0)) / gsl_vector_get(params, 1)) / (-2.0);
  case cMVNORMAL : // SHOULD BE EVALUATED BY A DIFFERENT FUNCTION
    MULTIVARIATE_DISTRIBUTION;
  case cUNIFORM : // FIRST PARAMETER IS THE LOWER BOUND, THE SECOND PARAMETER THE UPPER BOUND
    return (x < gsl_vector_get(params, 0) || x > gsl_vector_get(params, 1)) ? GSL_NEGINF : 0;
  default :
				      ...
				      const gsl_vector* params)
{
  ...
  case cCONSTANT :
    return 0;
  case cGAMMA :
    return gsl_sf_log(gsl_ran_gamma_pdf(x, gsl_vector_get(params, 0), 1 / gsl_vector_get(params, 1)));
  case cBETA :
  ...
  case cMVNORMAL : // SHOULD BE EVALUATED BY A DIFFERENT FUNCTION
    MULTIVARIATE_DISTRIBUTION;
  case cUNIFORM : // FIRST PARAMETER IS THE LOWER BOUND, THE SECOND PARAMETER THE UPPER BOUND
    return gsl_sf_log(gsl_ran_flat_pdf(x, gsl_vector_get(params, 0), gsl_vector_get(params, 1)));
  default :
    DEFAULT_NO_DISTRIBUTION;
  }
...
double R_univariate_prior_log_density_ratio(const double& x1,
					    const double& x2,
					    const distribution_type& dist,
					    const gsl_vector* params)
{
  ...
  case cCONSTANT :
    return 0;
  case cGAMMA : // FIRST PARAMETER IS THE SHAPE PARAMETER, SECOND PARAMETER IS THE RATE PARAMETER
    return ((gsl_vector_get(params, 0) - 1) * gsl_sf_log(x1 / x2)) - (gsl_vector_get(params, 1) * (x1 - x2));
  case cBETA : // FIRST PARAMETER IS THE FIRST SHAPE PARAMETER 'a',
    // SECOND PARAMETER IS THE SECOND SHAPE PARAMETER, 'b'
    return (((gsl_vector_get(params, 0) - 1) * gsl_sf_log(x1 / x2))
	    + ((gsl_vector_get(params, 1) - 1) * gsl_sf_log((1 - x1) / (1 - x2))));
  case cNORMAL : // FIRST PARAMETER IS THE MEAN, SECOND PARAMETER IS THE STANDARD DEVIATION
  case cHALFNORMAL : // SEE ABOVE
    return (-1 / (2 * gsl_pow_2(gsl_vector_get(params, 1)))) * (gsl_pow_2(x1 - gsl_vector_get(params, 0)) - gsl_pow_2(x2 - gsl_vector_get(params, 0)));
  case cMVNORMAL : // SHOULD BE EVALUATED BY A DIFFERENT FUNCTION
    MULTIVARIATE_DISTRIBUTION;
  case cUNIFORM : 
    flag1 = (x1 >= gsl_vector_get(params, 0)) && (x1 <= gsl_vector_get(params, 1));
    flag2 = (x2 >= gsl_vector_get(params, 0)) && (x1 <= gsl_vector_get(params, 1));
    return (flag1) ? ((flag2) ? 0 : GSL_POSINF) : ((flag2) ? GSL_NEGINF : 0); // CHECK GSL_INF....
  default :
    DEFAULT_NO_DISTRIBUTION;
  }
...
int num_parameters_by_distribution(const distribution_type& dist, const int& dimension)
{
  switch(dist) {
  case cCONSTANT :
    return 0;
...
double R_proposenew2(double& newparam,
		    const double& oldparam,
		    const double& propvar,
		    gsl_rng* r,
		    const char& trunc_flag,
		    ...
		    const double& trunc_up = 1.0)
{
  ...
  double newparam_trans = param_trans + gsl_ran_gaussian_ziggurat(r, proposenew_sd);
  // Inverse transformation
  newparam = inverse_rw_transformation(newparam_trans, trunc_flag, trunc_lo, trunc_up);
...
double R_proposenew(double& newparam,
		    const double& oldparam,
		    const double& propvar,
		    gsl_rng* r,
		    const char& trunc_flag,
		    ...
		    const double& trunc_up = 1.0)
{
  // proposes a new parameter value newparam, from a random walk proposal with mean oldparam and variance propvar. 
  // The function returns the proposal acceptance ratio, equal to the ratio of the two normalisations where these are 
  // applicable (in the case of truncation), else 1. Optional arguments should indicate the lower and upper limits of the truncation respectively
  ...
  if(trunc_flag)
      ...
      if((trunc_flag==cTRUNC) || (trunc_flag==cTRUNC_UP))
	{
      ...
      // COMPUTATION OF CDF AT THE RIGHT AND LEFT BOUNDARIES OF THE TRUNCATED DISTRIBUTION
      devb = (trunc_flag==cTRUNC_LO) ? 1.0 : gsl_cdf_ugaussian_P(proposenew_upper);
      deva = (trunc_flag==cTRUNC_UP) ? 0.0 : gsl_cdf_ugaussian_P(proposenew_lower);
    } 
  else 
    {  
      devb=1.0;
  ...
  // COMPUTATION OF THE RATIO OF THE PROPOSED GIVEN THE CURRENT OVER THE CURRENT GIVEN THE PROPOSED PROPOSAL DISTRIBUTIONS
  if(trunc_flag)
    {
      if((trunc_flag==cTRUNC) || (trunc_flag==cTRUNC_UP))
	{
	  if(newparam > trunc_up) newparam = trunc_up - DBL_EPSILON;
	  proposenew_upper = (trunc_up - newparam)/proposenew_sd;
	}
      if((trunc_flag==cTRUNC) || (trunc_flag==cTRUNC_LO))   
	{
	  if(newparam < trunc_lo) newparam = trunc_lo + DBL_EPSILON;
...
double variance_inflation_factor(const double& proposal_variance, const distribution_type& dist, const gsl_vector* prior_params)
{
  ...
  case cUNIFORM :
    return FN_MIN(proposal_variance, MAX_ZERO_ONE_PROPOSAL_VARIANCE * (gsl_vector_get(prior_params, 1) - gsl_vector_get(prior_params, 0)));
  case cGAMMA :
  case cNORMAL : 
  case cMVNORMAL :
  case cHALFNORMAL :
...
double R_inverse_link_function(double x,
			       const link_function g)
{
...
void R_generalised_linear_regression(gsl_vector* out_y, const gsl_matrix *design_X,
				     const gsl_vector* param_beta, link_function g)
{
  ...
  for(int int_i = 0; int_i < out_y->size; int_i++)
    gsl_vector_set(out_y,
		   int_i,
		   R_inverse_link_function(gsl_vector_get(out_y,
							  int_i),
					   g));
...
void R_by_sum_gsl_vector_mono_idx(gsl_vector* oVec, gsl_vector* oVec_wts, const gsl_vector* iVec, const vector<int>& idx_breaks)
{  // Aggregates the population size over observed strata (oVec) and outputs if not NULL the weights of each input strata within the
  // observed strata.
  ...
  if(iVec->size < idx_breaks.size()){
    cout << "R_by_sum_stl_vector_mono_idx: Indices exceed maximum length of input vector." << endl;
  ...
  for(int int_i = 0; int_i < oVec->size; itIdx++){
    ...
    gsl_vector_set(oVec, int_i++, gsl_vector_sum_elements(&iVec_sub.vector));
    if(oVec_wts != NULL){
      gsl_vector_view oVec_wts_sub = gsl_vector_subvector(oVec_wts, offset, int_width);
      gsl_vector_memcpy(&oVec_wts_sub.vector, &iVec_sub.vector);
      gsl_vector_scale(&oVec_wts_sub.vector, 1 / gsl_vector_get(oVec, int_i - 1));
    }
    offset += int_width;
    int_width = (((itIdx + 1) == idx_breaks.end()) ? iVec_cpy->size : *(itIdx + 1)) - *itIdx;
  }
