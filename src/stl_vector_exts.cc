...
template <typename D>
void stl_vector_sscanf(const string &in_string, vector<D> &out_vec,
    ...
    for (int i = 0; i < int_vec_length; i++)
        out_vec.push_back(
            read_from_delim_string<D>(in_string, chr_delim, indx));
}

template void stl_vector_sscanf<int>(const string &, vector<int> &,
                                     const char *);
template void stl_vector_sscanf<double>(const string &, vector<double> &,
                                        const char *);
