...
struct global_model_instance_parameters {
    data_type
        l_gp_count_likelihood; // DO WE HAVE POISSON OR NEGATIVE BINOMIAL DATA?
    data_type l_hosp_count_likelihood; // AS ABOVE
    transmission_kernel
        l_tk; // REED-FROST OR MASS-ACTION. REED-FROST BY DEFAULT.
    int l_transmission_time_steps_per_day; // DELTA T FOR THE TRANSMISSION MODEL
    int l_reporting_time_steps_per_day;    // DELTA_T FOR THE REPORTING MODEL.
                                           // SHOULD BE LESS THAN
                                           // l_transmission_time_steps_per_day
    ...
    int l_start_week;
    gsl_vector_int *d_week_numbers_by_day; // THE WEEK NUMBER FOR EACH DAY OF
                                           // THE STUDY. CALCULATED BASED ON THE
                                           // ABOVE TWO MEMBERS OF THE STRUCTURE
    int l_num_regions;
    int l_GP_consultation_flag; // FLAGS INDICATING PRESENCE/ABSENCE OF DATA
                                // TYPES, THIS ROW DOWN
    ...
    int l_GP_patch_flag;
    int l_Sero_delay; // NUMBER OF DAYS OF LAG BUILT INTO SEROLOGY DATA
                      // REPRESENTING THE TIME TAKEN FOR IMMUNOLOGICAL RESPONSE
    likelihood_bounds
        l_GP_likelihood; // WHICH DAYS/WEEKS OF DATA TO USE IN CALCULATING THE
                         // LIKELIHOOD, THIS ROW DOWN
...
struct updateable_model_parameter {
    ...
    gsl_vector_int
        *prior_distribution; // THE DISTRIBUTION OF THE INDIVIDUAL COMPONENTS
    ...
    gsl_vector **prior_params; // WHAT THESE PARAMETERS REPRESENT DEPENDS ON THE
                               // prior_distribution MEMEMBER OF THE STRUCTURE.
                               // EACH ROW CORRESPONDS TO EACH OF THE
                               // DISTRIBUTIONS IN THE prior_distribution VECTOR
    ...
    bool flag_transmission_model; // DOES THE NUMBER OF NEW INFECTEDS NEED TO
                                  // RECALCULATED WHEN UPDATING THE PARAMETER
                                  // THIS PARAMETER
    bool
        flag_reporting_model; // DO THE REPORTING DELAYS NEED TO BE RECALCULATED
                              // WHEN UPDATING WHEN UPDATING THIS PARAMETER
    bool flag_GP_likelihood;  // DOES THE LIKELIHOOD FOR G.P. CONSULTATION NEED
                              // TO BE RECALCULATED WHEN UPDATING THIS PARAMETER
    bool flag_Hosp_likelihood; // DOES THE LIKELIHOOD FOR THE HOSPITALISATIONS
                               // NEED TO BE RECALCULATED WHEN UPDATING THIS
                               // PARAMETER
    bool flag_Sero_likelihood; // DOES THE LIKELIHOOD FOR THE SEROEPIDEMIOLOGY
                               // DATA NEED TO BE RECALCULATED WHEN UPDATING
                               // THIS PARAMETER
    bool flag_Viro_likelihood; // DOES THE LIKELIHOOD FOR THE VIROLOGY
                               // (POSITIVITY) DATA NEED TO BE RECALCULATED WHEN
                               // UPDATING THIS PARAMETER
    bool flag_any_child_nodes; // TRUE IF ANY FLAG_CHILD_NODES ARE TRUE. FALSE
                               // OTHERWISE
    bool *flag_child_nodes;    // FLAG FOR EACH OF THE OTHER PARAMETERS OF THE
                            // MODEL INDICATING WHETHER THEY ARE CHILD NODES OF
                            // THE CURRENT NODE
};
...
struct mixing_model {
    int num_breakpoints;
    gsl_vector_int
        *breakpoints; // TIMING OF BREAKPOINTS, ASSUMED ORDERED AND INCREASING.
    gsl_matrix **MIXMAT;
...
void mixing_model_alloc(mixing_model &, const int, const int,
                        const int num_strata = NUM_AGE_GROUPS);
void mixing_model_free(mixing_model &);
void mixing_model_memcpy(mixing_model &, const mixing_model &);

// PARAMETERS DEFINING THE GAMMA DISTRIBUTION GOVERNING DELAY TIMES IN THE
// DISEASE MODEL
struct st_delay {
    string delay_name;
    double gamma_mean;
...
struct infection_to_data_delay {
    ...
    gsl_vector *distribution_function; // CURRENTLY DISTRIBUTIONS ARE FIXED OVER
                                       // AGES AND REGIONS, SO ONLY NEED THE
                                       // TIME DIMENSION HERE
};
...
// GLOBAL PARAMETER STRUCTURE, FOR INPUT TO THE TRANSMISSION (AND DISEASE?)
// MODEL
...
// REGION SPECIFIC PARAMETER STRUCTURE, FOR INPUT TO THE TRANSMISSION (AND
// DISEASE?) MODEL
#define REGIONAL_MODEL_PARAMS_MEMBERS                                          \
    "l_init_prop_sus, l_init_prop_sus_HI_geq_32, "                             \
    "l_average_infectious_period, l_latent_period, "                           \
    "l_relative_infectiousness_I2_wrt_I1, l_EGR, l_R0_Amplitude, "             \
    "l_R0_peakday, l_R0_init, l_I0, l_pr_symp, l_pr_onset_to_GP, "             \
    "l_pr_onset_to_Hosp, l_pr_onset_to_Death, l_importation_rate, "            \
    "d_R0_phase_differences, l_MIXMOD, l_background_gps_counts, "              \
    "l_sensitivity, l_specificity, l_gp_negbin_overdispersion, "               \
    "l_hosp_negbin_overdispersion, l_day_of_week_effect;"
struct regional_model_params {
    gsl_vector *l_init_prop_sus; // INITIAL CONDITION, MAKES NO SENSE TO HAVE
                                 // ANY TEMPORAL VARIATION
    gsl_vector *l_init_prop_sus_HI_geq_32; // INITIAL CONDITION, MAKES NO SENSE
                                           // TO HAVE ANY TEMPORAL VARIATION
    gsl_matrix
        *l_average_infectious_period; // TEMPORALLY VARYING TO ALLOW FOR CHANGES
                                      // DUE TO ACCESS TO ANTIVIRALS
    ...
    double l_R0_init; // INITIAL CONDITION, MAKES NO SENSE TO HAVE ANY TEMPORAL
                      // VARIATION. AGE_VARIATION IS ON HOLD
    double l_I0;      // INITIAL CONDITION, MAKES NO SENSE TO HAVE ANY TEMPORAL
                      // VARIATION. AGE VARIATION IS ON HOLD
...
void regional_model_params_alloc(regional_model_params &, const int, const int,
                                 const int, const int, const mixing_model);
void regional_model_params_memcpy(regional_model_params &,
                                  const regional_model_params &, flagclass &);
void regional_model_params_free(regional_model_params &);

// STRUCTURE CONTAINING OUTPUTS
struct model_statistics {
    model_state
        *d_end_state;  // should contain a value for S(T, a), E_1(T, a), E_2(T,
                       // a), I_1(T, a), I_2(T, a) for all a = 1, ..., NUM_AGES
...
void model_statistics_aggregate(gsl_matrix *, const model_statistics &,
                                const int);
void model_statistics_memcpy(model_statistics &, const model_statistics,
                             bool NNI_flag = true, bool GP_flag = true,
                             bool Hosp_flag = true, bool Sero_flag = true,
                             bool Viro_flag = true);
...
void Region_alloc(Region &, const global_model_instance_parameters,
                  const mixing_model);
