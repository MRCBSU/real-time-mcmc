...
/// Returns a tuple of {input directory, output directory}, which will be
/// strings. Prints help string and exits if that is requested. Also causes exit
...
std::tuple<string, string> parse_command_line_arguments(int argc, char **argv) {
    ...
    args::ValueFlag<string> input_dir(parser, "input",
                                      ...
                                      "files mod_pars.txt and mod_input.txt",
                                      {'i', "input"}, "inputs");
    args::ValueFlag<string> output_dir(
        parser, "output",
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        DEBUG(DEBUG_ERROR,
              "Cannot read " << str_filename_inputs << " :" << e.what());
    ...
    string str_global_model_parameters_initvals(
        GLOBAL_MODEL_PARAMETERS_DEFAULT_FIXED_VALS);
    string str_global_model_parameters_likflags(
        GLOBAL_MODEL_PARAMETERS_LIKELIHOOD_FLAGS);
    ...
    try {
        // BELOW FUNCTION WILL ALLOC MEMORY TO GLOBAL_MODPARS, AND SET TO FILE
        // SPECIFIED VALUES OR DEFAULTS
        read_global_model_parameters(
            global_modpars, str_filename_modpars.c_str(),
            str_global_model_parameters_members,
            str_global_model_parameters_initvals,
            str_global_model_parameters_likflags,
            str_global_model_delay_members, str_global_model_delay_means,
            str_global_model_delay_sds, str_global_model_delay_flags,
            global_fixedpars.l_num_regions,
            global_fixedpars.l_duration_of_runs_in_days, NUM_AGE_GROUPS,
            global_fixedpars.l_reporting_time_steps_per_day);
    } catch (std::fstream::failure e) {
        DEBUG(DEBUG_ERROR,
              "Cannot read " << str_filename_modpars << " :" << e.what());
        exit(2);
    }
    // //
    ...
    for (int int_i = 0; int_i < global_fixedpars.l_num_regions; int_i++)
        evaluate_regional_parameters(
            country[int_i].det_model_params, global_modpars.param_list,
    ...
    read_mcmc_parameters(sim_pars, str_filename_inputs.c_str(),
                         str_mcmc_parameter_names, str_mcmc_parameter_defaults);
...
// SET THE MAXIMUM NUMBER OF PARALLEL THREADS
    ...
    fn_log_likelihood(
        llhood, country, 0, true, true, global_fixedpars.l_GP_consultation_flag,
        global_fixedpars.l_Hospitalisation_flag,
        global_fixedpars.l_Viro_data_flag, global_fixedpars, global_modpars);

    // RUN THE METROPOLIS-HASTINGS SAMPLER AND SEND OUTPUT TO FILES
    metrop_hast(sim_pars, global_modpars, country, llhood, global_fixedpars,
                mixmod_struct, sim_pars.r, output_dir);
    ...
    // FREE THE MIXING MODEL STRUCTURE INTO WHICH THE MATRICES WERE INITIALLY
    // READ
