...
#pragma omp parallel for default(shared) schedule(static)
	for (int reg = 1; reg <= nregions; reg++) {
	  paramSet.blocks[reg].calcRegionLhood(paramSet, country2, gmip, prop_lfx.rlik[reg-1]);
	}
      ...
      if(int_iter >= simulation_parameters.burn_in && !((int_iter + 1 - simulation_parameters.burn_in) % simulation_parameters.thin_stats_every))
	{
	  for(int int_reg = 0; int_reg < nregions; int_reg++)
	    {
	      model_statistics_aggregate(output_NNI[int_reg], // Overloaded function, might need an IF statement to control
					 output_Delta_Dis[int_reg],
