...
void likelihood_alloc(likelihood &new_llhood,
    ...
    new_llhood.GP_lfx =
        (gmip.l_GP_consultation_flag) ? gsl_vector_alloc(num_regions) : 0;
    if (new_llhood.GP_lfx != 0)
        gsl_vector_set_zero(new_llhood.GP_lfx);
    new_llhood.Hosp_lfx =
        (gmip.l_Hospitalisation_flag) ? gsl_vector_alloc(num_regions) : 0;
    if (new_llhood.Hosp_lfx != 0)
        gsl_vector_set_zero(new_llhood.Hosp_lfx);
    new_llhood.Deaths_lfx =
        (gmip.l_Deaths_flag) ? gsl_vector_alloc(num_regions) : 0;
    if (new_llhood.Deaths_lfx != 0)
        gsl_vector_set_zero(new_llhood.Deaths_lfx);
    new_llhood.Sero_lfx =
        (gmip.l_Sero_data_flag) ? gsl_vector_alloc(num_regions) : 0;
    if (new_llhood.Sero_lfx != 0)
        gsl_vector_set_zero(new_llhood.Sero_lfx);
    new_llhood.Viro_lfx =
        (gmip.l_Viro_data_flag) ? gsl_vector_alloc(num_regions) : 0;
    if (new_llhood.Viro_lfx != 0)
        gsl_vector_set_zero(new_llhood.Viro_lfx);
}
...
void mcmcPars_free(mcmcPars &out_mP) { gsl_rng_free(out_mP.r); }

void alloc_global_model_instance(
    global_model_instance_parameters &in_globalModelInstanceParameters) {
    in_globalModelInstanceParameters.d_week_numbers_by_day =
        gsl_vector_int_alloc(
            in_globalModelInstanceParameters.l_duration_of_runs_in_days);
}

void free_global_model_instance(
    global_model_instance_parameters &in_globalModelInstanceParameters) {
...
void infection_to_data_delay_alloc(infection_to_data_delay &new_itdd,
                                   size_t num_delays, size_t num_times) {
    ...
    if (num_times > 0)
        ...
        new_itdd.distribution_function = 0;
}
...
// ---- overloaded regional_model_params_alloc function; allocation of memory
// for regional_model_params_alloc objects.
void regional_model_params_alloc(regional_model_params &new_rmp,
    ...
    new_rmp.l_average_infectious_period =
        gsl_matrix_calloc(transmission_time_steps_per_day * num_days,
                          num_ages); /// DO I WANT NUM_DAYS AND NUM_AGES? OR THE
                                     /// NUMBER OF TEMPORAL AND AGE BREAKPOINTS.
    new_rmp.l_latent_period = gsl_matrix_calloc(
        transmission_time_steps_per_day * num_days,
        num_ages); /// THE SAME GOES FOR MANY OF THE OTHER MATRICES
    new_rmp.l_relative_infectiousness_I2_wrt_I1 =
        gsl_matrix_calloc(transmission_time_steps_per_day * num_days,
                          num_ages); // VARIATION NOT EXPECTED TO BE USED HERE
    new_rmp.l_pr_symp =
        gsl_matrix_calloc(reporting_time_steps_per_day * num_days, num_ages);
    new_rmp.l_pr_onset_to_GP =
        gsl_matrix_calloc(reporting_time_steps_per_day * num_days, num_ages);
    new_rmp.l_pr_onset_to_Hosp =
        gsl_matrix_calloc(reporting_time_steps_per_day * num_days, num_ages);
    new_rmp.l_pr_onset_to_Death =
        gsl_matrix_calloc(reporting_time_steps_per_day * num_days, num_ages);
    new_rmp.l_importation_rate =
        gsl_matrix_calloc(transmission_time_steps_per_day * num_days, num_ages);
    new_rmp.d_R0_phase_differences =
        gsl_vector_calloc(transmission_time_steps_per_day * num_days);

    int num_mix_breakpoints = (src_mixing_pars.breakpoints == 0)
                                  ? 0
                                  : src_mixing_pars.breakpoints->size;

    // GET THE SIZE OF THE PARAMETER VECTOR FROM THE MAXIMAL INDEX FOUND IN THE
    // PARAMETERISATION MATRICES
    int param_vec_size = maximal_index(src_mixing_pars) + 1;

    mixing_model_alloc(new_rmp.l_MIXMOD, num_mix_breakpoints, param_vec_size);
    ...
    new_rmp.l_hosp_negbin_overdispersion =
        gsl_matrix_calloc(num_days, num_ages);
...
void regional_model_params_alloc(regional_model_params &dest_rmp,
    ...
    dest_rmp.l_init_prop_sus_HI_geq_32 =
        gsl_vector_calloc(src_rmp.l_init_prop_sus_HI_geq_32->size);
    dest_rmp.l_average_infectious_period =
        gsl_matrix_calloc(src_rmp.l_average_infectious_period->size1,
                          src_rmp.l_average_infectious_period->size2);
    dest_rmp.l_latent_period = gsl_matrix_calloc(
        src_rmp.l_latent_period->size1, src_rmp.l_latent_period->size2);
    dest_rmp.l_relative_infectiousness_I2_wrt_I1 =
        gsl_matrix_calloc(src_rmp.l_relative_infectiousness_I2_wrt_I1->size1,
                          src_rmp.l_relative_infectiousness_I2_wrt_I1->size2);
    dest_rmp.l_pr_symp =
        gsl_matrix_calloc(src_rmp.l_pr_symp->size1, src_rmp.l_pr_symp->size2);
    dest_rmp.l_pr_onset_to_GP = gsl_matrix_calloc(
        src_rmp.l_pr_onset_to_GP->size1, src_rmp.l_pr_onset_to_GP->size2);
    dest_rmp.l_pr_onset_to_Hosp = gsl_matrix_calloc(
        src_rmp.l_pr_onset_to_Hosp->size1, src_rmp.l_pr_onset_to_Hosp->size2);
    dest_rmp.l_pr_onset_to_Death = gsl_matrix_calloc(
        src_rmp.l_pr_onset_to_Death->size1, src_rmp.l_pr_onset_to_Death->size2);
    dest_rmp.l_importation_rate = gsl_matrix_calloc(
        src_rmp.l_importation_rate->size1, src_rmp.l_importation_rate->size2);
    dest_rmp.d_R0_phase_differences =
        gsl_vector_calloc(src_rmp.d_R0_phase_differences->size);

    mixing_model_alloc(dest_rmp.l_MIXMOD, src_rmp.l_MIXMOD.num_breakpoints,
                       src_rmp.l_MIXMOD.scalants->size,
                       src_rmp.l_MIXMOD.evector_MIXMAT_normalised[0]->size);
    dest_rmp.l_background_gps_counts =
        gsl_matrix_calloc(src_rmp.l_background_gps_counts->size1,
                          src_rmp.l_background_gps_counts->size2);
    dest_rmp.l_gp_negbin_overdispersion =
        gsl_matrix_calloc(src_rmp.l_gp_negbin_overdispersion->size1,
                          src_rmp.l_gp_negbin_overdispersion->size2);
    dest_rmp.l_hosp_negbin_overdispersion =
        gsl_matrix_calloc(src_rmp.l_hosp_negbin_overdispersion->size1,
                          src_rmp.l_hosp_negbin_overdispersion->size2);

    dest_rmp.l_day_of_week_effect =
        gsl_matrix_calloc(src_rmp.l_day_of_week_effect->size1,
                          src_rmp.l_day_of_week_effect->size2);
}
...
void regional_model_params_memcpy(regional_model_params &rmp_dest,
                                  const regional_model_params &rmp_src,
    ...
    if (update_flags.getFlag("l_init_prop_sus_HI_geq_32"))
        gsl_vector_memcpy(rmp_dest.l_init_prop_sus_HI_geq_32,
                          rmp_src.l_init_prop_sus_HI_geq_32);
    if (update_flags.getFlag("l_average_infectious_period"))
        gsl_matrix_memcpy(rmp_dest.l_average_infectious_period,
                          rmp_src.l_average_infectious_period);
    ...
    if (update_flags.getFlag("l_relative_infectious_period"))
        gsl_matrix_memcpy(rmp_dest.l_relative_infectiousness_I2_wrt_I1,
                          rmp_src.l_relative_infectiousness_I2_wrt_I1);
    ...
    if (update_flags.getFlag("l_pr_onset_to_Hosp"))
        gsl_matrix_memcpy(rmp_dest.l_pr_onset_to_Hosp,
                          rmp_src.l_pr_onset_to_Hosp);
    if (update_flags.getFlag("l_pr_onset_to_Death"))
        gsl_matrix_memcpy(rmp_dest.l_pr_onset_to_Death,
                          rmp_src.l_pr_onset_to_Death);
    if (update_flags.getFlag("l_importation_rate"))
        gsl_matrix_memcpy(rmp_dest.l_importation_rate,
                          rmp_src.l_importation_rate);
    if (update_flags.getFlag("d_R0_phase_differences"))
        gsl_vector_memcpy(rmp_dest.d_R0_phase_differences,
                          rmp_src.d_R0_phase_differences);
    ...
    if (update_flags.getFlag("l_background_gps_counts"))
        gsl_matrix_memcpy(rmp_dest.l_background_gps_counts,
                          rmp_src.l_background_gps_counts);
    ...
    if (update_flags.getFlag("l_gp_negbin_overdispersion"))
        gsl_matrix_memcpy(rmp_dest.l_gp_negbin_overdispersion,
                          rmp_src.l_gp_negbin_overdispersion);
    if (update_flags.getFlag("l_hosp_negbin_overdispersion"))
        gsl_matrix_memcpy(rmp_dest.l_hosp_negbin_overdispersion,
                          rmp_src.l_hosp_negbin_overdispersion);
    if (update_flags.getFlag("l_day_of_week_effect"))
        gsl_matrix_memcpy(rmp_dest.l_day_of_week_effect,
                          rmp_src.l_day_of_week_effect);
}

void model_statistics_alloc(model_statistics &ms, const int times,
                            const int age_classes,
                            const int reporting_timesteps) {
...
void model_statistics_aggregate(gsl_matrix *output_NNI,
                                const model_statistics &ms,
                                const int contraction) {
    output_per_selected_period(contraction, ms.d_NNI, output_NNI);
}

void model_statistics_memcpy(model_statistics &ms_dest,
                             const model_statistics ms_src, bool NNI_flag,
                             bool GP_flag, bool Hosp_flag, bool Sero_flag,
                             bool Viro_flag) {
    *ms_dest.d_end_state = *ms_src.d_end_state;
    if (NNI_flag)
        gsl_matrix_memcpy(ms_dest.d_NNI, ms_src.d_NNI);
    if (GP_flag) {
        gsl_matrix_memcpy(ms_dest.d_Reported_GP_Consultations,
                          ms_src.d_Reported_GP_Consultations);
        gsl_matrix_memcpy(ms_dest.d_H1N1_GP_Consultations,
                          ms_src.d_H1N1_GP_Consultations);
    }
    if (Hosp_flag)
        gsl_matrix_memcpy(ms_dest.d_Reported_Hospitalisations,
                          ms_src.d_Reported_Hospitalisations);
    if (Sero_flag)
...
void Region_alloc(Region &new_reg,
    ...
    if (src_gmip.l_GP_consultation_flag)
        new_reg.GP_data = new rtmData(src_gmip.l_GP_likelihood,
                                      src_gmip.l_gp_count_likelihood);
    else
        new_reg.GP_data = 0;
    if (src_gmip.l_Hospitalisation_flag)
        new_reg.Hospitalisation_data = new rtmData(
            src_gmip.l_Hosp_likelihood, src_gmip.l_hosp_count_likelihood);
    else
        new_reg.Hospitalisation_data = 0;
    if (src_gmip.l_Deaths_flag)
        new_reg.Death_data =
            new rtmData(src_gmip.l_Deaths_likelihood, cPOISSON);
    else
        new_reg.Death_data = 0;
    if (src_gmip.l_Sero_data_flag)
        new_reg.Serology_data =
            new rtmData(src_gmip.l_Sero_likelihood, cBINOMIAL);
    else
        new_reg.Serology_data = 0;
    if (src_gmip.l_Viro_data_flag)
        new_reg.Virology_data =
            new rtmData(src_gmip.l_Viro_likelihood, cBINOMIAL);
    else
        new_reg.Virology_data = 0;

    // FUNCTIONS ALLOCATING THE regional_model_params AND model_statistics
    // STRUCTURES NEEDED HERE
    regional_model_params_alloc(new_reg.det_model_params, num_days, num_ages,
                                src_gmip.l_transmission_time_steps_per_day,
                                src_gmip.l_reporting_time_steps_per_day,
                                src_mixmod);
    model_statistics_alloc(new_reg.region_modstats, num_days, num_ages,
                           src_gmip.l_reporting_time_steps_per_day);
}

// ---- alternate form of the above
void Region_alloc(Region &new_reg, const Region &old_reg) {
    new_reg.data_owner = false;
    regional_model_params_alloc(new_reg.det_model_params,
                                old_reg.det_model_params);
    model_statistics_alloc(new_reg.region_modstats,
                           ...
                           old_reg.region_modstats.d_NNI->size1 /
                               old_reg.region_modstats.d_seropositivity->size1);
}
void Region_free(Region &old_reg,
                 const global_model_instance_parameters src_gmip) {

    if (old_reg.data_owner) {
        gsl_vector_free(old_reg.population);
        if (src_gmip.l_GP_consultation_flag)
            delete old_reg.GP_data;
        if (src_gmip.l_Hospitalisation_flag)
            delete old_reg.Hospitalisation_data;
        if (src_gmip.l_Deaths_flag)
            delete old_reg.Death_data;
        if (src_gmip.l_Sero_data_flag)
            delete old_reg.Serology_data;
        if (src_gmip.l_Viro_data_flag)
            delete old_reg.Virology_data;
    }
    // FUNCTIONS FREEING THE regional_model_params AND model_statistics
    // STRUCTURES NEEDED HERE
    regional_model_params_free(old_reg.det_model_params);
    model_statistics_free(old_reg.region_modstats);
}

void Region_memcpy(Region &reg_dest, const Region &reg_src,
                   flagclass &update_flags) {
    ...
    if (reg_src.GP_data !=
        0) // Datasets should be invariant, so ok to equate pointers.
    ...
    regional_model_params_memcpy(reg_dest.det_model_params,
                                 reg_src.det_model_params, update_flags);
