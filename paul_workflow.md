...
=======
   b. Set ``hosp.flag <- 0``. Set ``data.desc <- "admissions"``. Data on admissions are downloaded from Seb Funk's website at LSHTM, downloaded as an `.rds` file and then renamed to give it a date equal to the latest date in the data +1 day. This file should be saved, or linked to, from the directory ``data/raw/admissions`` within the repo. On line 11, I set ``args`` to be equal to ``today() - days(n)`` where ``n`` is the number of days prior to today that the admissions dataset is dated. Update variable `date.adm_seb` with the date of the new data file.
