...
should be lifted off the JMT shared area. Date formats often change - so might need to check the setting of the variable ``sero.date.fmt``. All data with date beyond ``sero.end.date`` will be ignored, so make sure it is appropriately set. Set variable `sero.end.date` to be the same as `sero.end.1stwv` to use only first wave data, use the day of the run to use all available swabbing dates.'
