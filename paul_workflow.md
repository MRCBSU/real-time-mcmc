...
2. Serological data - should be lifted off the JMT shared area. Date formats often change - so might need to check the setting of the variable ``sero.date.fmt``. All data with date beyond ``sero.end.date`` will be ignored, so make sure it is appropriately set.
