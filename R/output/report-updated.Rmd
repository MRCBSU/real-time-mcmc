...
knitr::opts_chunk$set(echo = FALSE, cache.path = file.path(out.dir,"tmp", "cache"))
...
Dta.text <- ifelse(deaths.flag, "Deaths", "Admissions")
severity.measure <- ifelse(deaths.flag, "IFR", "IHR")
```
...
We fit a transmission model ([Birrell et al. 2020](https://www.medrxiv.org/content/10.1101/2020.08.24.20180737v1)) to a number of data sources (see 'Data Sources'), to  reconstruct the number of  new COVID-19 infections over time in different age groups and ONS regions, estimate a measure of ongoing transmission and predict the number of new hospital admissions associated with COVID-19.
...
6. Data on new diagnoses in hospital testing. This is a composite dataset derived from two sources:
...
```{r attack-region}
df.AR <- as.tbl_cube(AR, met_name = "value") %>%
      as_tibble() %>%
      mutate(date = lubridate::as_date(date)) %>%
      filter(date == lubridate::as_date(date.data)) %>%
      left_join(population, by = c('age' = 'age', 'region' = 'region')) %>%
      rename(AR = value.x, population = value.y)
df.AR.Eng <- df.AR %>%
	  group_by(age, iteration) %>%
	  summarise(AR = weighted.mean(AR, w = population), population = sum(population)) %>%
	  mutate(region = "England")

tbl.AR.by.region <- bind_rows(df.AR %>%
      group_by(iteration, region) %>%
      summarise(AR = weighted.mean(AR, w = population)),
      df.AR.Eng %>%
      group_by(iteration, region) %>%
      summarise(AR = weighted.mean(AR, w = population))
      ) %>%
      group_by(region) %>%
      summarise(value = quantile(AR, probs = c(0.025, 0.5, 0.975))) %>%
      mutate(quantile = c(0.025, 0.5, 0.975), value = paste0(signif(100 * value, 3), "%")) %>%
      pivot_wider(region, names_from = quantile) %>%
      rename(Region = region) %>%
      transmute(`Attack rate` = paste0(`0.5`, "(", `0.025`, "--", `0.975`, ")"))

bind_rows(filter(tbl.AR.by.region, Region == "England"), filter(tbl.AR.by.region, Region != "England")) %>%
		 datatable()
```
...
tbl.AR.by.age <- df.AR %>%
	      ...
	      summarise(value = quantile(AR, probs = c(0.025, 0.5, 0.975))) %>%
      	      mutate(quantile = c(0.025, 0.5, 0.975), value = paste0(signif(100 * value, 3), "%")) %>%
      	      pivot_wider(age, names_from = quantile) %>%
      	      transmute(`Attack rate` = paste0(`0.5`, "(", `0.025`, "--", `0.975`, ")")) %>%
	      mutate(Age = factor(age, levels = age.labs, labels = age.labs, ordered = TRUE)) %>%
	      ungroup() %>%
	      select(Age, `Attack rate`) %>%
	      arrange(Age)

datatable(tbl.AR.by.age)	      
...
calc.infec.growth.rate <- function(R,gt,infecs) {
  ...
  gt <- do.call(cbind, gt) %>% cbind(apply((.), 1, mean))
  rt <- R %>%
...
if(!exists("Egt")) load(file.path(out.dir, "forSPI.RData"))

infections.growth.rates <- calc.infec.growth.rate(extract(Rt, indices = list(as.character(today.int)), dims = "date", drop = FALSE),
						Egt,
						extract(infections, indices = list(as.character(today.int)), dims = "date", drop = FALSE))
...
### Change in `r dta.text` incidence {.tabset}
...
cat("The execution of the prevalence code block will proceed if\n prev.dat exists and this is", !is.null(prev.dat), "\n it is not an external report and this is", !external, "\n") 
```

## Infections and hospitalisations {.tabset}

The shaded areas show periods of national lockdown, the green lines the dates (once confirmed) of the steps in the roadmap in the [UK Government's COVID-19 Response -- Spring 2021](https://www.gov.uk/government/publications/covid-19-response-spring-2021), and the red line shows the date these results were produced (`r format(ymd(date.data), "%d %b")`).

```{r opt-prev-report, eval=((!external)&&(!is.null(prev.dat))), child='prev-report.Rmd'}
```
...
```{r IFR-plot, cache=FALSE, cache.extra=results_hash}

source(file.path(Rfile.loc, "ifr_format.R"))
...
```{r P-R-gt-1, cache=FALSE, cache.extra=results_hash}
(Rt > 1) %>%
...
### $R_t$
```{r Rt-plot, cache=FALSE, cache.extra=results_hash}
