...
```{r combine-Rt}
inf_by_region <- apply(infections, c("iteration", "date", "region"), sum)
...
make.plots <- function(projections, ylab = "", by = NULL, data = NULL,
  ...
  create.subplot <- function(projections, subplot_title) {
    projections %>%
  ...
  plots <- list("England" = create.subplot(Eng_projection, "England"))
  for(subplot in plot.names) {
    plots[[subplot]] <- projections %>%
      filter(by == subplot) %>%
      create.subplot(subplot)
    if (!is.null(data)) {
      plot.data <- data %>%
...
params$prop_case_to_hosp %>%
  ...
  #arrange(match(age, age.labs)) %>%
  plot_ly(
    x = ~factor(age, levels = dimnames(deaths)$age),
    y = ~ifr
  ) %>%
  add_boxplot() %>%
  layout(yaxis = list(tickformat = "%", title = "IFR (infection fatality rate)",
                      hoverformat = ".2%"),
