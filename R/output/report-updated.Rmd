...
Analysis from latest data shows:

* There has been a decrease in the number of daily deaths in every region, with a particularly sharp decrease in London.
* The COVID-19 epidemic peaked across all regions around the time of the lockdown. Until this time, the London epidemic had the highest levels of infection because infection spread in this region earlier than elsewhere.
* The proportion of people who have already been infected differs by region and it is highest in London, where the central estimate is around 20%. These proportions are lower in other regions (around 5-14%), consistently with the different timing of the epidemic in each region.
* The estimated number of infections in each age group are derived through the use of a model that tracks the susceptible fraction of the population and uses contact survey data that describe relative rates of contacts between different age groups [Mossong et al. 2008](https://journals.plos.org/plosmedicine/article?id=10.1371/journal.pmed.0050074). Data from Google on changes in people’s mobility and the ONS’ time use survey then help to inform us on how these contact rates change over time, particularly after the lockdown.
* Measures of transmission (R) also differ between regions. R represents the average number of onward infections that will be generated by a single infectious individual. Epidemics will grow if R&gt;1, but this quantity does not indicate how fast the epidemic will grow, and therefore should not be used as the sole indicator of the current threat posed by an epidemic.
* Regions currently estimated to have the highest value for R (North East, South West, East of England) are those which had the lowest level of activity prior to the lockdown, due to a delayed spread into these regions.
