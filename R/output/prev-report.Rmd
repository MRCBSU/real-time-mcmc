...
#### By region
```{r reg-prevalence, cache=FALSE, cache.extra=results_hash}
...
reg_deonm = population %>%
	group_by(name=region) %>%
	summarise(denom=sum(value))
age_deonm = population %>%
	group_by(name=age) %>%
	summarise(denom=sum(value))
make.plots(prev_plot, by = "region", data = prev.dat.plot, denom = reg_deonm)

```

#### By age
```{r age-prevalence}}
make.plots(prev_plot, by = "age", data = prev.dat.plot, denom = age_deonm)
```
