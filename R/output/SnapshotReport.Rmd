...
summary_simple_state_ages <- state |> 
  ...
  summarise(value = sum(value), popn = sum(popn)) |> 
  mutate(value = ifelse(popn > 0, value / popn * 100, 0)) |> 
...
p1 <- summary_simple_state_ages |> 
  ...
  mutate(state.names= factor(state.text, levels = state.levels, labels = names(state.levels), ordered = T)) |>
  arrange(state.names, dose.numbers) |> 
