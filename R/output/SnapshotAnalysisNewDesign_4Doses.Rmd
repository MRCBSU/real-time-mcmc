...
Analysis of a snapshot of the results from the Real-time Nowcasting and Forecasting model with input data until 2022-03-25 and the model state on 2022-03-25.
...
# nhs_file <- "NHS_England_Regions_(April_2020)_Boundaries_EN_BGC/NHS_England_Regions_(April_2020)_Boundaries_EN_BGC.shp"
# ons_file <- "Regions_(December_2020)_EN_BFC/Regions_(December_2020)_EN_BFC.shp"
...
# state<- state |> 
  # expand_grid(dose = c(0,1,2,3))

# summary_simple_state <- state |> 
#   group_by(region, state.names, age.labs, popn, state.text) |> 
#   summarise(mean = mean(value), n = n(), upp_ci95 = mean + (qnorm(0.975) * sd(value) / sqrt(n())), low_ci95 = mean - (qnorm(0.975) * sd(value) / sqrt(n())))
# 
# summary_simple_state |> 
#   filter(state.names != "plambda") |> 
#   mutate(mean = mean / popn,  upp_ci95 = upp_ci95 / popn, low_ci95 = low_ci95 / popn) |> 
#   filter(age.labs == "75+") |> 
#   ggplot(aes(x = state.names, y = mean * 100, ymax = upp_ci95 * 100, ymin = low_ci95 * 100, color = region)) +
#   geom_errorbar() +
#   facet_wrap(~state.names, scales = "free_y")
# 
# summary_state_correct <- state |>
#   group_by(region, state.names, age.labs, popn, state.text) |> 
#   summarise(min = min(value), max = max(value), v = quantile(value, c(0.025, 0.5, 0.975)), q = c(0.025, 0.5, 0.975)) |> 
#   mutate(q = factor(q, levels = c(0.025, 0.5, 0.975), labels = c("low_ci95", "ce", "upp_ci95"))) |> 
#   ungroup() |> 
#   pivot_wider(id_cols = c(region, state.names, popn, min, max, state.text, age.labs), values_from = v, names_from = q)
# 
# summary_state_correct|> 
#   filter(state.names != "plambda") |> 
#   mutate(ce = ce / popn,  upp_ci95 = upp_ci95 / popn, low_ci95 = low_ci95 / popn) |> 
#   filter(age.labs == "75+") |> 
#   ggplot(aes(x = "value", y = ce * 100, ymax = upp_ci95 * 100, ymin = low_ci95 * 100, color = region)) +
#   geom_errorbar(position = position_dodge(width = 1.1)) +
#   facet_wrap(~state.names, scales = "free_y")
# 
# summary_state_correct_2 <- state |>
#   group_by(iteration, region, age.labs, popn, state.text) |> 
#   
#   summarise(min = min(value), max = max(value), v = quantile(value, c(0.025, 0.5, 0.975)), q = c(0.025, 0.5, 0.975)) |> 
#   mutate(q = factor(q, levels = c(0.025, 0.5, 0.975), labels = c("low_ci95", "ce", "upp_ci95"))) |> 
#   ungroup() |> 
...
# state$state.text |>  unique()
# 

# state |>  filter(state.text == "Infectious pressure")
```
...
# state |>  
#   filter(state.names != "plambda") |> 
#   group_by(state.names, region, state.text, age.labs, dose.numbers, popn) |> 
#   summarise(mean = mean(value)) |> 
#   group_by(region, age.labs, popn) |> 
#   summarise(sum = sum(mean))
# 

# summary_simple_state_ages$state_text_old |>  unique()
...
summary_simple_state_ages <- 
  ...
  summarise(value = sum(value), popn = sum(popn)) |> 
  mutate(value = data.table::fifelse(value == 0 & popn == 0, 0,value / popn * 100)) |> 
...
combined_state_levels <- c(
  ...
  "Susceptible, Never Infected, 4+ Vaccine Doses" ,
  ...
  "Susceptible, Previously Infected 4+ Vaccine Doses"
)
...
p1 <- summary_simple_state_ages |> 
  ...
  mutate(dose.numbers = factor(dose.numbers, levels = vacc_levels, labels = names(vacc_levels), ordered = T)) |> 
  mutate(state.names= factor(state.text, levels = state.levels, labels = names(state.levels), ordered = T)) |> arrange(state.names, dose.numbers) |> 
  mutate(ce = data.table::fifelse(popn == 0, 0, ce / popn * 100), low_ci95 = data.table::fifelse(popn == 0, 0, low_ci95 / popn * 100), upp_ci95 = data.table::fifelse(popn == 0, 0,upp_ci95 / popn * 100)) |> 
  # unite("state.names", state.names, dose.numbers, sep = " ") |> 
  # mutate(state.names = factor(state.names, levels = combined_state_levels, ordered = T)) |> 
ggplot(aes(x = age.labs, y = ce, ymin = low_ci95, ymax = upp_ci95, color = age.labs, group = age.labs, text = paste0("State: ", state.names, "\nAge: ", age.labs, "\nProportion of Population: ", scales::label_number(accuracy = 0.01)(ce), "%\nUpper Estimate: ", scales::label_number(accuracy = 0.01)(upp_ci95), "%\nLower Estimate: ", scales::label_number(accuracy = 0.01)(low_ci95), "%"))) +
    geom_col(aes(fill = after_scale(alpha(colour, 0.6)))) +
...
summary_simple_state_ages <- state |> 
  ...
  summarise(value = sum(value)) |> 
  mutate(value = data.table::fifelse(popn == 0, 0, value / popn * 100)) |> 
...
combined_state_levels <- c(
  ...
  "Infected Non-infectious 4+ Vaccine Doses" ,
  ...
  "Infected Infectious 4+ Vaccine Doses"
)
...
summary_simple_state_ages <- state |> 
  ...
  filter(!is.na(state.names)) |> 
  group_by(state.names, age.labs, iteration, dose.numbers, popn) |> 
  summarise(value = data.table::fifelse(popn == 0, 0,sum(value) / popn * 100)) |> 
  group_by(state.names, age.labs, dose.numbers) |> 
  summarise(v = quantile(value, c(0.025, 0.5, 0.975)), q = c(0.025, 0.5, 0.975)) |> 
  mutate(q = factor(q, levels = c(0.025, 0.5, 0.975), labels = c("low_ci95", "ce", "upp_ci95"))) |> 
  ungroup() |> 
...
combined_state_levels <- c(
  ...
  "Previously Infected 4+ Vaccine Doses"
)
...
if(region.type == "ONS") sf_df <- read_rds("ons_region_sf.rds")
if(region.type == "NHS") sf_df <- read_rds("nhs_region_sf.rds")
