...
#hosp.flag<-1
#data.desc<-"deaths"
#Rfile.loc = "/home/aa995/rds/hpc-work/real-time-mcmc/R/output"
#proj.dir = "/rds/user/aa995/hpc-work/real-time-mcmc"
...
if(gp.flag & nA>1){
    ...
    pars.pgp <- rbind(pars.pgp[, 1], sqrt(diag(pars.pgp[, -1]))) %>%
        as.vector()
}
if(gp.flag){
    pars.dow <- rep(c(0, sqrt(5.357143)), 6)
} else pars.dow <- NA
oudim = 10
var.priors <- list(distribution = list(NULL,
                                       NULL,
                                       NULL,
                                       rep(list(dgamma), gp.flag),
                                       rep(list(dgamma), hosp.flag),
                                       NULL,
                                       list(dgamma),
                                       list(dgamma),
                                       NULL,
                                       ...
                                       #rep(lapply(1:12, function(j) {if(j != 1) return(dnorm) else return(NULL)}), nr),
                                       NULL,
                                       NULL,
                                       rep(list(dgamma), r),
                                       rep(list(dnorm),  r),
                                       NULL,
                                       NULL,
                                       rep(switch(gp.flag + 1, NULL, ifelse(nA == 1, list(dbeta), list(dnorm))), length(value.pgp)),
                                       rep(switch(hosp.flag + 1, NULL, ifelse(single.ifr, list(dbeta), list(dnorm))), length(value.ifr)),
                                       NULL,
                                       NULL,
                                       NULL,
                                       NULL,
...
## ## DRAW CODA PLOTS
pdf(file.path(out.dir, "codas_angelos.pdf"))
par(mfrow = c(1, 2))
for(inti in 1:npar)
  {
    start.index <- 1
...
###########Transform ODE parameters to RW process#############################

#source(file.path(Rfile.loc, "lsoda.R")) #load Colin's functions
