...
if(Use_preprocessed_serology) {
    preprocessed_sero_rdata_loc <- file.path(data.dirs["sero"], "..", "..", "serology_preprocessed_rdatas", paste0("sero_data_", preprocessed_sero_date,".RData"))
    if(!(all(file.exists(serosam.files)) & all(file.exists(seropos.files)) & file.exists(preprocessed_sero_rdata_loc))) {

        # print(serosam.files)
        # print(file.exists(serosam.files))
        # print(seropos.files)
        # print(file.exists(seropos.files))
        # print(preprocessed_sero_rdata_loc)
        # print(file.exists(preprocessed_sero_rdata_loc))
        stop("Some datafiles are missing that should have been created by the preprocessing (serology)")
    }

    load(preprocessed_sero_rdata_loc)
    ...
    write_csv(rtm.sam, file.path(data.dirs["sero"], paste0("sero_samples_data", ".csv")))
    write_csv(rtm.pos, file.path(out.dir, "sero_positives_data.csv"))
    # Rename files if using a custom cutoff
    write_csv(rtm.pos, file.path(data.dirs["sero"], paste0("sero_positives_data", ".csv")))
