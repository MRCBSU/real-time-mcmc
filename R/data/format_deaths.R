...
if(!use_deaths_up_to_now_flag) length_of_deaths_dates <- as.numeric(difftime(custom_deaths_end_date, earliest.date,"days"))

if(Use_preprocessed_deaths) {
    preprocessed_deaths_rdata_loc <- file.path(data.dirs["deaths"], "..", "..", "deaths_preprocessed_rdatas", paste0("deaths_data_", date.data, "cutoff", str.cutoff,".RData"))
    if(!(all(file.exists(data.files)) & file.exists(preprocessed_deaths_rdata_loc))) {

        print(data.files)
        print(preprocessed_deaths_rdata_loc)

        stop("Some datafiles are missing that should have been created by the preprocessing (deaths)")
    }
    load(preprocessed_deaths_rdata_loc)
    if(region.type == "NHS") {
        dth.dat <- dth.dat.nhs
        rtm.dat <- rtm.dat.nhs
    } else {
