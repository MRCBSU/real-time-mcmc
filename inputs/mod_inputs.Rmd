...
Hospitalisation_flag `r as.numeric(hosp.flag | adm.flag)`
...
Prev_data_flag	     `r prev.flag`

Vacc_data_flag	     `r vacc.flag`

VBoost_data_flag     `r ifelse(vac.n_doses >= 3, 1, 0)`

VFourth_data_flag     `r ifelse(vac.n_doses >= 4, 1, 0)`
...
Prev_likelihood_start_day	   `r start.prev`

Prev_likelihood_end_day		   `r end.prev`

Vacc_start_day		   `r start.vac`

Vacc_end_day		   `r end.vac`

VBoost_start_day	   `r start.vac`

VBoost_end_day		   `r end.vac`

VFourth_start_day	   `r start.vac`

VFourth_end_day		   `r end.vac`
...
study_region = { regions_used = `r paste(regions, collapse = ", ")`;
		 ...
		 `r ifelse(adm.flag, "regions_hosp_aggregation = 4, 5, 6, 7;", "")`
		 ...
		 regions_logprevalence_estimates = `r ifelse(prev.flag, paste(prev.data$lmeans, collapse = ", "), "NULL")`;
		 regions_logprevalence_sd = `r ifelse(prev.flag, paste(prev.data$lsds, collapse = ", "), "NULL")`;

		 regions_1stvaccination_data = `r ifelse(vacc.flag, paste(vac1.files, collapse = ", "), "NULL")`;
		 regions_2ndvaccination_data = `r ifelse(vacc.flag, ifelse(vac.n_doses >= 3, paste(vac2.files, collapse = ", "),
		 			       	  		    		       	     paste(vacn.files, collapse = ", ")),
								    "NULL")`;

		 regions_boostvaccination_data = `r ifelse(vacc.flag & vac.n_doses >= 3, paste(vac3.files, collapse = ", "), "NULL")`;

		 regions_4thvaccination_data = `r ifelse(vacc.flag & vac.n_doses >= 4, paste(vac4.files, collapse = ", "), "NULL")`;
...
adapt_every	     `r adapt.every`

burn_in		     `r burnin`

read_covar	     `r stored.covar`

global_updates	     `r global.per.iter`
