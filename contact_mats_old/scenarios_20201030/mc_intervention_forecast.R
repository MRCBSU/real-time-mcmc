...
mm.breaks <- ymd("20201026") + (1:nforecast.weeks * days(7))
google.data.date <- ymd("20201030")
intervention.breaks <- ymd("20201105", "20201119")
...
cm.bases <- c(cm.bases[1:first(idx)],
            file.path(proj.dir, "contact_mats", paste0("england_8ag_contact_intervention", "_", format(google.data.date, "%Y%m%d"), ".txt")),
...
save(list = save.list, file = "projections2wk_lockdown.RData")
